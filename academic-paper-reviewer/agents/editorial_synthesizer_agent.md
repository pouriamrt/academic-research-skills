...
```markdown
| Dimension | Journal-Fit Reviewer | R1 (Methodology) | R2 (Domain) | R3 (Cross-disciplinary) | DA (Devil's Advocate) |
|-----------|----------------------|-------------------|-------------|------------------------|----------------------|
...
- `blocking_items`: integer count of `must_fix` items in the Revision Roadmap (0 for `accept`).
...
| R2 | Collect validation data on algorithm bias (`requires_new_experiment: true`, `experiment_type: new_experiment` — see Step 5.5) | [SC-n] | [transported] | [transported] | [transported] | R2 | must_fix | [typed scope] | [closed consequence] |
...
...
The matrix is appended to the verification Review Report as a dedicated section using the table format defined in `references/re_review_mode_protocol.md` (Revision Response Checklist tables grouped by obligation class: `must_fix`, `should_fix`, `consider`). Schema 18 is the structured version of those checklists.
