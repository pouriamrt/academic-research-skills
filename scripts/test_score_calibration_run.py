...
def write_panel(
    runs_dir: Path, paper: str, replicate: int, decision: str, synthesis: str | None = None
) -> None:
...
def run(tmp_path: Path, runs_dir: Path, gold: Path, **kw) -> tuple[int, dict | None]:
    ...
    argv = [
        ...
        str(out),
        "--replicates",
        str(kw.pop("replicates", 3)),
    ]
    for flag, value in kw.items():
        argv += [f"--{flag.replace('_', '-')}", str(value)]
    rc = mod.main(argv)
    return rc, json.loads(out.read_text(encoding="utf-8")) if out.is_file() else None
...
def standard(tmp_path):
    ...
    plan = {
        "a1": [
            ...
            "Major Revision",
            "Accept",
        ],  # majority negative, gold accept -> FN
...
def test_invalid_override_cannot_hide_behind_successful_extraction(standard):
    ...
    overrides.write_text(
        json.dumps({"a1-r1": {"decision": "Accept", "raw": "not in the synthesis"}}),
        encoding="utf-8",
    )
...
def test_severity_histogram_computed(standard):
    rows = [
        {"panel": "a1-r1", "weakness_id": "w1", "risk": "high"},
