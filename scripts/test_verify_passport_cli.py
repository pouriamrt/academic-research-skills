...
import yaml  # noqa: E402  import follows the sys.path bootstrap above
...
def _no_network_clients():
    ...
    return {n: default() for n in ("crossref", "openalex", "semantic_scholar", "arxiv")}
...
def test_cli_synthetic_ref_slug_uses_citation_key(tmp_path, capsys):
    ...
    rc = run(
        [str(passport), "--synthetic-ref-slug", "citation_key"], clients_factory=_no_network_clients
    )
    assert rc == 0
