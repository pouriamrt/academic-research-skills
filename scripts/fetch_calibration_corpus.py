...
def main() -> int:
    ...
    if out_path.is_file():
        existing = {
            p["paper_id"]: p for p in json.loads(out_path.read_text(encoding="utf-8"))["papers"]
        }
    ...
    for cls, pid in ids:
        ...
        print(
            f"ok   {cls} {pid} n={rec['number']} decision={rec['decision_raw']!r} pdf_bytes={size}"
        )
        out_path.write_text(
            json.dumps(
                {"venue_id": VENUE_ID, "papers": list(papers.values())},
                indent=2,
                ensure_ascii=False,
            ),
            encoding="utf-8",
