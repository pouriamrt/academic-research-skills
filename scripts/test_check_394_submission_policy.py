...
def test_tp_schema_widened_enum_fails(tp_schema):
    mutated = json.loads(json.dumps(tp_schema))
    mutated["properties"]["submission_package"]["enum"].append("strict_articles_only")
...
def test_formatter_lost_literal_fails(formatter_text, literal):
    ...
    section_re = re.compile(
        r"^## Submission Package Advisories.*?(?=^## |\Z)", re.MULTILINE | re.DOTALL
    )
    mutated = section_re.sub(lambda m: m.group(0).replace(literal, "REDACTED"), formatter_text)
...
def test_verifier_real_tree_is_single_homed():
    assert check_verifier_single_homed(VERIFIER.read_text(encoding="utf-8")) == []
...
def test_subscript_access_fires():
    assert find_terminal_policies_access('x = passport["terminal_policies"]\n')


def test_dotget_access_fires():
    assert find_terminal_policies_access('x = passport.get("terminal_policies", {})\n')
...
def test_report_schema_widened_slug_enum_fails(report_schema):
    ...
    mutated["properties"]["header"]["properties"]["policy_slug"]["enum"].append("paranoid")
...
def test_report_schema_dropped_null_semantics_fails(report_schema):
    mutated = json.loads(json.dumps(report_schema))
    mutated["properties"]["header"]["properties"]["policy_slug"]["description"] = "a slug"
    errs = check_report_schema(mutated)
    assert errs
