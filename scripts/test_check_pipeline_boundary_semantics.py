...
class PipelineBoundarySemanticsTests(unittest.TestCase):
    ...
    def test_inv1_orchestrator_blueprint_dropped(self) -> None:
        mutated = self.orch.replace(
            "RQ Brief, Annotated Bibliography, Synthesis Report, Methodology Blueprint, Concept Lineage Report",
            "RQ Brief, Annotated Bibliography, Synthesis Report, Concept Lineage Report",
        )
    ...
    def test_inv2_skill_routing_rerouted(self) -> None:
        ...
        mutated = self.skill.replace(
            ...
            "6. **Stage 3' RE-REVIEW** -> Accept -> Stage 4.5 / Minor|Major -> **Experiment Re-Entry Check** -> Stage 4' (last experiment opportunity)",
        )
    ...
    def test_inv2_coaching_trigger_widened_to_minor(self) -> None:
        ...
        handoff_mut = self.orch.replace(
            "| Stage 3' -> **experiment check** -> **coaching** -> 4' | New Revision Roadmap (if Major) |",
            "| Stage 3' -> **experiment check** -> **coaching** -> 4' | New Revision Roadmap (if Minor/Major) |",
        )
    ...
    def test_inv1_tracker_prereq_blueprint_dropped(self) -> None:
        ...
        mutated = self.tracker.replace(
            "| Stage 2 | None (but Stage 1 output recommended) | RQ Brief, Methodology Blueprint, Bibliography, Synthesis; Experiment Design + Results + Lab Record (if Stage 1.5 executed) |",
    ...
    def test_inv4_tracker_stage6_dropped(self) -> None:
        ...
        mutated = self.tracker.replace(
            ...
            '"1", "1.5a", "1.5b", "1.5c", "2", "2.5", "3", "4", "3p", "4p", "4.5", "5"',
        )
        self.assertNotEqual(mutated, self.tracker)
        errors = self._check(tracker=mutated)
        self.assertTrue(
