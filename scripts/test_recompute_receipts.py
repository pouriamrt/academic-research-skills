...
def receipt_lines(procedure: str, fields: dict[str, str]) -> dict[str, str]:
    text = rr.compute_receipts(rr.parse_extraction(one_request(procedure, fields)))
    parsed = {}
...
def test_p_inequality_threshold_graze_is_unresolvable():
    ...
    fields = receipt_lines(
        ...
        {
            "test_family": "z",
            "statistic_value": "1.9599639845400545",
            "df": "none",
            "reported_p_comparator": "less_than",
            "reported_p_value": ".05",
            "tail_convention": "unstated",
...
def test_p_sentinel_mappings():
    ...
    for override, reason in cases:
        fields = receipt_lines("p_from_test_statistic", {**P_FIELDS, **override})
        assert fields["status"] == "not_computable"
...
def test_grimmer_consistent_sd():
    fields = receipt_lines("grimmer", {**GRIMMER_FIELDS, "reported_sd": "0.47"})
    assert fields["status"] == "consistent"
...
def test_n_from_df_consistent_equals():
    fields = receipt_lines(
        ...
        {
            "df_reported": "39",
            "df_identity_candidate": "df=N-1",
            "stated_n": "40",
            "stated_n_relation": "equals",
        },
    )
    assert fields["status"] == "consistent"
...
def test_decorated_and_fenced_machine_lines_parse():
    body = (
        one_request("n_from_df", N_FROM_DF_FIELDS)
        .replace("procedure_id: n_from_df", "- **procedure_id**: n_from_df")
        .replace("df_reported: 156", "```\ndf_reported: 156\n```")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda t: t.replace("procedure_id: grim", "procedure_id: grimm"), "procedure_id"),
        ...
        (lambda t: t.replace("reported_mean: 3.847", "reported_mean: 3,847"), "plain decimal"),
...
def full_worked_extraction() -> str:
    ...
    for index, (procedure, fields) in enumerate(
        ...
        parts.append("evidence_anchor: table: Table 1, reported statistics under test")
...
def test_every_emitted_line_parses_under_the_gate_grammar():
    ...
    for line in text.splitlines()[1:]:
        ...
        if line.startswith("### "):
            ...
            continue
        matched = [
            key for key, pattern in phase._RECEIPT_FIELD_RES.items() if pattern.fullmatch(line)
        ]
...
import math  # noqa: E402  upstream keeps this next to the round-1 block it serves
...
def test_tail_functions_terminate_at_domain_corners():
    ...
    corners = [
        rr.t_tail_two(1e7, 10**7),
        rr.t_tail_two(1e7, 1),
        rr.t_tail_two(1e-7, 10**7),
        rr.z_tail_two(1e7),
        rr.z_tail_two(1e-7),
        rr.chi2_tail_upper(1e7, 10**7),
        rr.chi2_tail_upper(1e-7, 10**7),
        rr.chi2_tail_upper(1e7, 1),
        rr.f_tail_upper(1e7, 10**7, 10**7),
        rr.f_tail_upper(1e7, 1, 1),
        rr.f_tail_upper(1e-7, 10**7, 10**7),
        rr.f_tail_upper(1e-7, 1, 1),
    ]
