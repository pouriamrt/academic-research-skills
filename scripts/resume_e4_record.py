...
import _e4_evidence as evidence
import dispatch_e4_panel as harness
...
def _string(value, label: str, *, empty: bool = False) -> str:
    if not isinstance(value, str) or (not empty and not value.strip()):
        raise RecoveryError(f"{label} must be a string" + ("" if empty else " and non-empty"))
    return value
...
def _artifact_name(value, label: str) -> str:
    ...
    if path.is_absolute() or len(path.parts) != 1 or path.parts[0] in {".", ".."}:
        raise RecoveryError(f"{label} must name one record-bundle artifact, got {name!r}")
    return name
...
def _load_state(bundle_root: Path) -> dict:
    ...
    try:
        ...
        evidence.EvidencePathError,
        OSError,
        UnicodeDecodeError,
        json.JSONDecodeError,
    ) as failure:
        raise RecoveryError(
            f"cannot load {harness.RECOVERY_STATE_FILE}: {type(failure).__name__}: {failure}"
        ) from failure
    ...
    if state["evidence_contract"] != harness.EVIDENCE_CONTRACT:
        raise RecoveryError(f"unsupported evidence contract {state['evidence_contract']!r}")
    manifest = state["bundle_manifest"]
    if not isinstance(manifest, list) or not all(isinstance(entry, dict) for entry in manifest):
        raise RecoveryError("bundle_manifest must be a list of objects")
    if any(entry.get("type") not in {"file", "directory"} for entry in manifest):
        raise RecoveryError("bundle_manifest contains a redirected or non-plain artifact")
    current = evidence.tree_manifest(bundle_root, exclude={harness.RECOVERY_STATE_FILE})
    if current != manifest:
        raise RecoveryError(
            "preserved bundle manifest changed; refusing incomplete or inconsistent evidence"
        )
    return state


def _result_from_state(state: dict, bundle_root: Path) -> tuple[harness.PanelResult, dict]:
    ...
    if isinstance(replicate, bool) or not isinstance(replicate, int) or not 1 <= replicate <= 99:
    ...
    if not isinstance(canary, list) or not all(isinstance(item, str) for item in canary):
    ...
    result = harness.PanelResult(
        fixture, condition, replicate, completed_stages=list(completed), canary=list(canary)
    )
    for index, item in enumerate(retries):
        ...
        if stage not in {"phase1", "extraction", "phase2_multi_dissent", "synthesis"}:
            raise RecoveryError(f"unknown retry stage {stage!r}")
        diagnostic = _string(item["diagnostic"], f"result.retries[{index}].diagnostic", empty=True)
        ...
        checker = _artifact_name(
            item["checker_output_location"], f"result.retries[{index}].checker_output_location"
        )
        form = item["form"]
        ...
        checker_text = _read_plain_text(bundle_root, checker, "checker output")
        if diagnostic not in checker_text:
            raise RecoveryError(f"retry diagnostic is inconsistent with {checker}")
        result.retries.append(
            harness.RetryEvent(
                ...
                checker_output_location=checker,
                form=form,
            )
        )
    ...
    if abort is not None:
        ...
        diagnostic = _string(abort["diagnostic"], "result.abort.diagnostic", empty=True)
        log_name = _artifact_name(abort["log_name"], "result.abort.log_name")
        form = abort["form"]
        if form not in {None, "verbatim", "normalized"}:
            raise RecoveryError(f"unknown abort diagnostic form {form!r}")
        log_text = _read_plain_text(bundle_root, log_name, "abort checker output")
        if diagnostic not in log_text:
            raise RecoveryError(f"abort diagnostic is inconsistent with {log_name}")
        result.abort = harness.PanelAborted(stage, exit_code, diagnostic, log_name, form=form)

    for required in ("contract.json", "metadata.json", harness.Bundle.JOURNAL):
        _plain_file(bundle_root, required, required)
    journal = _read_plain_text(bundle_root, harness.Bundle.JOURNAL, "dispatch journal")
    journal_lines = journal.splitlines()
    witnessed_stages = [
        line.removeprefix("COMPLETE ") for line in journal_lines if line.startswith("COMPLETE ")
    ]
    if witnessed_stages != completed:
        raise RecoveryError("completed stages disagree with the append-only dispatch journal")
    try:
        ...
        UnicodeDecodeError,
        json.JSONDecodeError,
        harness.PreconditionFailure,
    ) as failure:
        raise RecoveryError(f"contract evidence is inconsistent: {failure}") from failure
    ...
    for line in journal_lines:
        ...
        response = _read_plain_text(bundle_root, artifact, "completed response")
        if len(response) != int(match.group(2)):
            raise RecoveryError(f"dispatch journal character count disagrees for {artifact}")
    ...
    if canary != expected_canary:
        raise RecoveryError("canary ledger disagrees with completed preserved responses")
    ...
    for event in result.retries:
        if event.stage == "synthesis":
            if event.role != "synthesis":
                raise RecoveryError("synthesis retry must be bound to the synthesis role")
            ...
            if event.role not in seats:
                raise RecoveryError(f"retry role is not a contract seat: {event.role!r}")
            if event.stage == "extraction" and event.role != "methodology":
                raise RecoveryError("extraction retry must be bound to the methodology seat")
            phase = {
                "phase1": "phase1",
                "extraction": "extraction",
            }.get(event.stage, "phase2")
            escaped = re.escape(event.role)
            response_pattern = rf"{escaped}\.{phase}\.a([1-9][0-9]*)\.md"
            checker_patterns = (rf"{escaped}\.{phase}\.a([1-9][0-9]*)\.gate\.log",)
            journal_prefix = {
                "phase1": f"RETRY {event.role} phase1 diagnostic=",
                "extraction": f"RETRY {event.role} extraction diagnostic=",
            }.get(event.stage, f"RETRY {event.role} phase2 multi-dissent diagnostic=")
        response_match = re.fullmatch(response_pattern, event.rejected_response_location)
        checker_match = next(
            (
                match
                for pattern in checker_patterns
                if (match := re.fullmatch(pattern, event.checker_output_location)) is not None
            ),
            None,
        )
        if (
            response_match is None
            or checker_match is None
            or response_match.group(1) != checker_match.group(1)
        ):
            raise RecoveryError("retry role/stage disagrees with its preserved artifact names")
        pair = (event.rejected_response_location, event.checker_output_location)
    ...
    journal_retry_lines = [line for line in journal_lines if line.startswith("RETRY ")]
    if witnessed_retry_lines != journal_retry_lines:
        raise RecoveryError("retry ledger disagrees with the append-only dispatch journal")
    ...
    if result.abort is not None:
        ...
        special = (
            ...
            and any(line.startswith(f"ABORT {result.abort.stage} ") for line in journal_lines)
        ) or (
            result.abort.exit_code == harness.EXIT_BLOCKED
            and (
                any(
                    line.startswith(f"ABORT transport {result.abort.stage} ")
                    for line in journal_lines
                )
        ...
        if exact not in journal_lines and not special:
            raise RecoveryError("terminal abort disagrees with the append-only dispatch journal")
    canonical = ["field_analysis"]
    for role in seats:
        canonical.append(f"{role}.phase1")
        if role == "methodology":
            # #610 step 5: the methodology seat's three-call shape.
            canonical.extend(("methodology.extraction", "methodology.recompute"))
        canonical.append(f"{role}.phase2")
    canonical.append("synthesis")
    positions = [canonical.index(stage) if stage in canonical else -1 for stage in completed]
    if -1 in positions or positions != sorted(positions):
        raise RecoveryError("completed stages are unknown or out of order")
    if result.abort is None and completed != canonical:
        raise RecoveryError("completed stages do not support a completed terminal record")
    ...
    for stage in completed:
        ...
        if stage == "methodology.recompute":
            ...
            for candidate in sorted(bundle_root.glob("methodology.extraction.a*.md")):
                gate = candidate.with_name(candidate.name.removesuffix(".md") + ".gate.log")
                try:
                    ...
                    continue
                terminal = next((line for line in reversed(gate_lines) if line.strip()), "")
            ...
            try:
                ...
                raise RecoveryError(
                    f"the preserved extraction no longer computes: {failure}"
                ) from failure
            actual_receipts = _read_plain_text(bundle_root, harness.RECEIPTS_ARTIFACT, stage)
        ...
        required_markers = (
            ...
            if stage == "methodology.phase2" and "methodology.recompute" in completed
            else ()
        )
        ...
        for candidate in candidates:
            ...
            terminal = next((line for line in reversed(gate_lines) if line.strip()), "")
            if terminal == pass_marker and all(marker in gate_lines for marker in required_markers):
                witnessed = True
                break
        if not witnessed:
            raise RecoveryError(f"completed stages lack response/checker evidence for {stage}")
...
def resume(bundle_path: Path, work_dir: Path) -> tuple[Path, dict]:
    ...
    blocked = result.abort is not None or result.provenance_status(bundle) != "valid"
    canonical_raw = (
        work_dir / "runs" / "raw" / "blocked" / stem
        if blocked
        else work_dir / "runs" / "raw" / stem
    )
    ...
    except (OSError, harness.PreconditionFailure, harness.PreservationError) as failure:
        raise RecoveryError(str(failure)) from failure
