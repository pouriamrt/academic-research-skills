...
class PreflightVerdictTest(unittest.TestCase):
    ...
    def test_parser_warnings_survive_early_exit(self):
        ...
        self.assertEqual(r["verdict"], "UNAVAILABLE", r)
        self.assertTrue(
            any(w == "pypdf: synthetic repair warning" for w in r["warnings"]), r["warnings"]
        )
...
class ContentClassificationSandboxTest(unittest.TestCase):
    ...
    def run_with_worker(self, worker, *, data=None, timeout=1.0, worker_env=None):
        ...
        schema = json.loads(
            (PDF_CONTRACTS / "pdf_read_preflight.schema.json").read_text(encoding="utf-8")
        )
    ...
    def test_operator_detail_byte_bound_survives_multibyte_cutoff(self):
        raw = b"x" * (preflight.CLASSIFIER_OPERATOR_DETAIL_LIMIT - 1) + "界".encode()
    ...
    def test_parent_never_imports_optional_native_classifier(self):
        source = (REPO_ROOT / "scripts" / "pdf_read_preflight.py").read_text(encoding="utf-8")
        ...
        self.assertIn(
            'importlib.import_module("pdf_inspector")', WORKER_PATH.read_text(encoding="utf-8")
        )
...
class CliTest(unittest.TestCase):
    ...
    def test_cli_input_resolve_failure_does_not_block_safe_output(self):
        ...
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assertEqual(json.loads(output.read_text(encoding="utf-8"))["verdict"], "UNAVAILABLE")
    ...
    def test_atomic_output_does_not_follow_postcheck_input_alias(self):
        ...
        for alias_kind in ("symlink", "hardlink"):
            with self.subTest(alias_kind=alias_kind):
                ...
                self.assertEqual(
                    json.loads(output.read_text(encoding="utf-8"))["marker"], alias_kind
                )
    ...
    def test_atomic_output_does_not_follow_postcheck_diagnostic_symlink(self):
        ...
        self.assertFalse(output.is_symlink())
        self.assertEqual(json.loads(output.read_text(encoding="utf-8")), {"marker": "sidecar"})
        self.assertEqual(
            json.loads(diagnostic_path.read_text(encoding="utf-8")), expected_diagnostic
        )
    ...
    def test_output_parent_symlink_retarget_uses_preworker_bound_directory(self):
        ...
        self.assertEqual(
            json.loads((original_parent / "sidecar.json").read_text(encoding="utf-8")),
            {"marker": "bound-parent"},
        )
    ...
    def test_diagnostic_parent_symlink_retarget_uses_preworker_bound_directory(self):
        ...
        self.assertEqual(
            json.loads((original_parent / "private.json").read_text(encoding="utf-8")),
            expected,
        )
    ...
    def test_diagnostic_partial_write_failure_removes_own_inode_and_allows_retry(self):
        ...
        try:
            ...
            bound.publish(payload)
            self.assertEqual(json.loads(diagnostic_path.read_text(encoding="utf-8")), payload)
        finally:
