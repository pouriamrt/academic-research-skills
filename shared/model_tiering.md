...
## Classification table (61 agents; upstream 39 frozen 2026-07-11 #517, fork +22 experiment-skill agents 2026-07-16)
...
| experiment-designer (1) | `design_architect` |
| simulation-runner (1) | `diagnostics` (convergence/validity assessment feeds design decisions) |
| lab-notebook (1) | `provenance_auditor` (audit authority over the research record) |

### Execution-type (32) — economy-direction downgrade candidates (one tier, floor Opus-class)

| Skill | Agents |
|---|---|
| deep-research (5) | `bibliography` (citation existence is handled by the deterministic verification gate, so the lookup layer does not depend on this agent's tier), `timeline_extraction`, `report_compiler`, `monitoring`, `concept_lineage` |
| academic-paper (5) | `intake`, `draft_writer` (highest-savings / most quality-sensitive — see Direction 2), `citation_compliance`, `visualization`, `formatter` (STAMP-ONLY by design) |
| academic-paper-reviewer (1) | `field_analyst` |
| experiment-designer (5) | `intake`, `power_analyst`, `instrument_builder`, `randomization`, `protocol_compiler` |
| data-analyst (7) | `intake`, `data_preparation`, `assumption_checker`, `analysis_executor`, `effect_size`, `visualization`, `report_compiler` |
| simulation-runner (4) | `intake`, `model_builder`, `execution_engine`, `report_compiler` |
| lab-notebook (3) | `notebook_manager`, `entry_writer`, `deviation_tracker` |
