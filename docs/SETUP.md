...
|---|---|---|---|
| `ARS_INTERACTIVE=1` | v3.17.0 | Restore v3.16.0 interactive checkpoint UX. When unset (default), the pipeline runs unattended: no `continue?` prompts, no beep, no mode-recommendation dialogue, no Stage 5 LaTeX prompt, no Stage 6 language picker; per-skill `socratic` / `plan` / `guided` modes are skipped. Stage 1.5 routing-flag MISSING defaults to `requires_experiment_design=false` with an advisory passport log. | `academic-pipeline/agents/pipeline_orchestrator_agent.md` §0 |
| `ARS_AUTO_MAX_RETRIES` | v3.17.0 | Cap on auto-retry rounds for Stage 2.5 integrity FAIL (default `3`). Stage 4.5 retry cap is hard-pinned at `1`. | `academic-pipeline/agents/pipeline_orchestrator_agent.md` §0 |
