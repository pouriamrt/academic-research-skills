...
def _is_windows_symlink_stand_in(entry: Path, expected: Path) -> bool:
    """True when `entry` is the checked-out stand-in for a mode-120000 blob.

    A stand-in is a small regular file whose entire content is the relative
    target path. Anything else — a directory, or a file with real content —
    is a genuine violation on every platform.
    """
    if entry.is_dir():
        return False
    try:
        ...
        return False
    return body in (f"../{entry.name}", entry.name) and expected.is_dir()
...
def _skills_dir_entries(root: Path, violations: list[str]) -> set[str]:
    ...
    for entry in sorted(skills_dir.iterdir(), key=lambda p: p.name):
        ...
        if not entry.is_symlink():
            # Windows checkouts with core.symlinks=false materialize a git
            # symlink as a regular file holding its target path. The index
            # entry is still mode 120000, so CI sees a real symlink; accept
            # that representation here instead of failing every local run.
            if _is_windows_symlink_stand_in(entry, expected):
                continue
            violations.append(
        ...
        if target != expected.resolve():
            violations.append(f"{entry}: symlink resolves to {target}, expected {expected}")
    return names
...
def _claude_table_rows(root: Path, violations: list[str]) -> set[str]:
    ...
    if section is None:
        violations.append(f"{claude_md}: '{SKILLS_OVERVIEW_HEADING}' H2 is missing")
        return set()
    rows: set[str] = set()
    ...
    if not rows:
        violations.append(
            f"{claude_md}: '{SKILLS_OVERVIEW_HEADING}' table has no backticked skill rows"
        )
    return rows
...
def _marketplace_skills(root: Path, violations: list[str]) -> tuple[set[str], list[str]]:
...
def _check_count_claim(label: str, text: str, expected: int, violations: list[str]) -> None:
...
def _report_set_diff(
    ...
    for name in sorted(other - on_disk):
        violations.append(f"{surface} lists skill '{name}' but no top-level {name}/SKILL.md exists")
...
def run_all_checks(root: Path) -> list[str]:
    ...
    if not on_disk:
        violations.append(f"{root}: no top-level <name>/SKILL.md found (wrong --path?)")
        return violations
    ...
    _report_set_diff(on_disk, table, f"{CLAUDE_MD} Skills Overview table", violations)

    manifest, market_descriptions = _marketplace_skills(root, violations)
    _report_set_diff(on_disk, manifest, f"{MARKETPLACE_JSON} plugins[].skills", violations)
    for description in market_descriptions:
        _check_count_claim(f"{MARKETPLACE_JSON} description", description, len(on_disk), violations)

    plugin = _load_json(root / PLUGIN_JSON, violations)
    if plugin is not None:
        description = plugin.get("description")
        if isinstance(description, str):
            _check_count_claim(f"{PLUGIN_JSON} description", description, len(on_disk), violations)
