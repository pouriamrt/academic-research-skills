...
def _validate_residual_gap(v: _V, gap, path):
    ...
    v.enum(
        gap["residual_obligation_class"],
        f"{path}.residual_obligation_class",
        RESIDUAL_OBLIGATION_CLASSES,
    )
...
def _validate_claim_authorizations(v: _V, authorizations, path):
    ...
    for i, authorization in enumerate(authorizations):
        ...
        v.pattern(
            authorization["author_event_id"],
            f"{row_path}.author_event_id",
            _AUTHOR_EVENT_RE,
            "AUTHOR-EVENT-<id>",
        )
        v.pattern(
            authorization["surface_id"],
            f"{row_path}.surface_id",
            _CLAIM_SURFACE_RE,
            "CLAIM-SURFACE-<id>",
        )
        v.pattern(
            authorization["scoped_manifest_id"],
            f"{row_path}.scoped_manifest_id",
            _MANIFEST_ID_RE,
            "scoped manifest id",
        )
        ...
        if (
            hashlib.sha256(authorization["replacement_text"].encode("utf-8")).hexdigest()
            != authorization["replacement_text_sha256"]
        ):
...
def _validate_tagged_anchors(v: _V, anchors, path):
    ...
    for i, entry in enumerate(anchors):
        ...
        v.pattern(
            entry["anchor"], f"{path}[{i}].anchor", _ANCHOR_RE, "the Schema 6 typed-anchor grammar"
        )
...
def validate_precommitment(art: dict):
    ...
    v.obj(
        art, "$", ["contract_version", "round_id", "input_manifest_hash", "items", "new_standards"]
    )
    ...
    for i, rec in enumerate(art["items"]):
        ...
        if has_text != has_ref:
            v.fail(
                f"{path}.inherited_criterion",
                "letter_text and letter_item_ref are biconditional (§5.1)",
            )
        if has_text:
            v.string(crit["letter_text"], f"{path}.inherited_criterion.letter_text")
            v.pattern(
                crit["letter_item_ref"],
                f"{path}.inherited_criterion.letter_item_ref",
                _LETTER_REF_RE,
                "R<n>",
            )
            if rec["obligation_class"] != "must_fix":
                v.fail(
                    f"{path}.inherited_criterion",
                    "letter fields exist ONLY on must_fix items (§5.1)",
                )
        ...
        v.obj(
            ...
            ["fully_addressed"],
            ["partially_addressed", "made_worse_discriminator"],
        )
        ...
        if rec["obligation_class"] == "must_fix":
            ...
            for field in ("partially_addressed", "made_worse_discriminator"):
                if field in op:
                    v.fail(
                        f"{path}.operationalization",
                        f"should_fix lighter form forbids {field!r} (§5.1)",
                    )
    ...
    for i, rec in enumerate(art["new_standards"]):
        ...
        v.obj(
            rec,
            path,
            ["new_standard_id", "item_id", "standard_text", "why_not_in_round1", "classification"],
        )
        ...
        v.enum(
            rec["classification"], f"{path}.classification", ("advisory", "escalation_requested")
        )
...
def validate_verdict_record(art: dict):
    ...
    for i, rec in enumerate(art["items"]):
        ...
        v.obj(
            rec,
            path,
            ["item_id", "verdict", "change_summary", "verified_by", "applied_criterion"],
        ...
        if rec["verdict"] == "CANNOT_VERIFY":
            if "cannot_verify_reason" not in rec or "evidence_anchor" in rec:
                v.fail(
                    path, "CANNOT_VERIFY carries cannot_verify_reason and no evidence_anchor (§3.3)"
                )
            ...
            if "evidence_anchor" not in rec or "cannot_verify_reason" in rec:
                v.fail(
                    path,
                    "non-CANNOT_VERIFY verdicts carry evidence_anchor and no cannot_verify_reason (§3.3)",
                )
    ...
    for i, rec in enumerate(art["dissents"]):
        ...
        v.enum(
            ...
            (
                "criterion_ambiguous",
                "criterion_infeasible_as_written",
                "evidence_surface_moved",
                "criterion_error",
            ),
        )
        for field in (
    ...
    for i, rec in enumerate(art["escalation_exceptions"]):
        ...
        v.enum(
            rec["mechanical_decision_impact"],
            f"{path}.mechanical_decision_impact",
...
def _validate_adjustment_body(v: _V, rec, path, *, drafted: bool):
    ...
    if not drafted:
        ...
        if "supersedes_adjustment_id" in rec:
            v.pattern(
                rec["supersedes_adjustment_id"],
                f"{path}.supersedes_adjustment_id",
                _ID_RES["ADJ"],
                "ADJ-<n>",
            )
    v.pattern(rec["item_id"], f"{path}.item_id", _ITEM_ID_RE, "REV-<id>")
    ...
    if rec["to_verdict"] == "CANNOT_VERIFY":
        if "cannot_verify_reason" not in rec or "evidence_anchor" in rec:
            v.fail(
                path, "to_verdict CANNOT_VERIFY carries cannot_verify_reason and no evidence_anchor"
            )
        v.string(rec["cannot_verify_reason"], f"{path}.cannot_verify_reason")
        if rec["basis"] != "user_accepted_fail_closed":
            v.fail(
                path,
                "user_accepted_fail_closed is the ONLY basis that may land on CANNOT_VERIFY (§3.4)",
            )
    else:
        if "evidence_anchor" not in rec or "cannot_verify_reason" in rec:
            v.fail(
                path,
                "non-CANNOT_VERIFY adjustments carry evidence_anchor and no cannot_verify_reason",
            )
    ...
    if rec["basis"] == "valid_rebuttal":
        ...
        if (
            rec["to_verdict"] not in ("PARTIALLY_ADDRESSED", "FULLY_ADDRESSED")
            or rank[rec["to_verdict"]] <= rank[rec["from_verdict"]]
        ):
            v.fail(
                path,
                "author_pointer_located_evidence adjustments are upgrades to PARTIALLY/FULLY_ADDRESSED (§3.4 Direction)",
            )
    if drafted:
        return
    ...
    if source_ref is not None:
        v.pattern(
            ...
            f"{path}.source_ref",
            _SOURCE_REF_RE,
            "reapplication:RAP-<n> | acceptance:ACC-<n>",
        )
    ...
    if check is not None:
        ...
        if check not in (
            "single_family_disclosed",
            "pass_unavailable_disclosed",
        ) and not _CHECK_ADJUDICATED_RE.match(check):
...
def validate_traceability(art: dict):
    ...
    for i, row in enumerate(art["rows"]):
        ...
        if has_status:
            v.enum(
                row["cross_model_status"],
                f"{path}.cross_model_status",
    ...
    for i, rec in enumerate(art["cross_model_resolutions"]):
        ...
        if rec["resolved_by"] == "user" and rec["state"] != "primary_upheld":
            v.fail(
                path,
                "the user acceptance form is always primary_upheld — the user never hand-sets a verdict (§5.3)",
            )
        v.string(rec["rationale"], f"{path}.rationale")
    ...
    for i, rec in enumerate(art["rebuttal_adjudications"]):
        ...
        v.pattern(
            rec["rebuttal_adjudication_id"],
            f"{path}.rebuttal_adjudication_id",
            _ID_RES["RADJ"],
            "RADJ-<n>",
        )
        v.pattern(rec["item_id"], f"{path}.item_id", _ITEM_ID_RE, "REV-<id>")
    ...
    for i, rec in enumerate(art["pending_rebuttal_upgrades"]):
        ...
        v.pattern(rec["item_id"], f"{path}.item_id", _ITEM_ID_RE, "REV-<id>")
        _validate_adjustment_body(
            v, rec["drafted_adjustment"], f"{path}.drafted_adjustment", drafted=True
        )
    ...
    for i, rec in enumerate(art["reapplications"]):
        ...
        for j, ref in enumerate(rec["answer_refs"]):
            v.pattern(
                ref,
                f"{path}.answer_refs[{j}]",
                _ANSWER_REF_RE,
                "adjudication:DIS-<n> | intent:INT-<n>",
            )
            if ref in seen_refs:
        ...
        if "supersedes_reapplication_id" in rec:
            v.pattern(
                rec["supersedes_reapplication_id"],
                f"{path}.supersedes_reapplication_id",
                _ID_RES["RAP"],
                "RAP-<n>",
            )
        ...
        if rec["reapplied_verdict"] == "CANNOT_VERIFY":
            ...
            if "evidence_anchor" not in rec or "cannot_verify_reason" in rec:
                v.fail(
                    path,
                    "non-CANNOT_VERIFY reapplications carry evidence_anchor and no cannot_verify_reason",
                )
            _validate_tagged_anchors(v, rec["evidence_anchor"], f"{path}.evidence_anchor")
        ...
        v.pattern(
            rec["criterion_ref"],
            f"{path}.criterion_ref",
            _CRITERION_REF_RE,
            "phase1:REV-<id> | dissent:DIS-<n>",
        )
    ...
    v.obj(
        ...
        [
            "per_item",
            "verdict_counts",
            "residual_obligation_class_counts",
    ...
    for i, entry in enumerate(di["per_item"]):
        ...
        v.obj(
            entry,
            path,
            ["item_id", "final_verdict", "driving_severity"],
            ["residual_obligation_class"],
        )
        ...
        if "residual_obligation_class" in entry:
            v.enum(
                entry["residual_obligation_class"],
                f"{path}.residual_obligation_class",
                RESIDUAL_OBLIGATION_CLASSES,
            )
    for bucket_name in ("verdict_counts", "residual_obligation_class_counts"):
        ...
        for prio in OBLIGATION_CLASSES:
            ...
            for key in keys:
                v.integer(
                    bucket[prio][key], f"$.decision_inputs.{bucket_name}.{prio}.{key}", minimum=0
                )
    ...
    v.integer(
        rate["denominator"], "$.decision_inputs.should_fix_addressed_rate.denominator", minimum=0
    )
    ...
    for i, entry in enumerate(di["non_regression_new_issue_ids"]):
        v.pattern(
            entry, f"$.decision_inputs.non_regression_new_issue_ids[{i}]", _ID_RES["NEW"], "NEW-<n>"
        )
    ...
    for i, entry in enumerate(di["escalations"]):
        ...
        v.enum(
            entry["effective_approval_state"],
            f"{path}.effective_approval_state",
            ("pending", "approved", "rejected"),
        )
        v.enum(entry["escalation_class"], f"{path}.escalation_class", ESCALATION_CLASSES)
        v.enum(
            entry["mechanical_decision_impact"],
            f"{path}.mechanical_decision_impact",
            ("Minor Revision", "Major Revision"),
        )
...
def validate_manifest(manifest: dict):
    ...
    def check_entry_fields(entry, key, path):
        ...
        if ref.startswith("path:"):
            ...
            if (
                ...
                fail(
                    f"{path}: path: refs carry a RELATIVE path — no absolute, drive-letter, or traversal segments (§11)"
                )
    ...
    for key in single_keys:
        ...
        if entry["present"]:
            if set(entry) != {
                "present",
                "path_or_passport_ref",
                "sha256",
                "version_label",
                "origin_date",
            }:
                fail(
                    f"artifacts.{key}: present entry carries exactly ref/sha256/version_label/origin_date"
                )
            check_entry_fields(entry, key, f"artifacts.{key}")
        elif set(entry) != {"present"}:
            fail(
                f"artifacts.{key}: an absent artifact carries NO ref, hash, or freshness fields (§11)"
            )
        ...
        if entry["present"]:
            ...
            for i, element in enumerate(items):
                if not isinstance(element, dict) or set(element) != {
                    ...
                    fail(
                        f"artifacts.{key}[{i}]: every element FULLY populated (no per-element absence)"
                    )
    ...
    for key in (
        ...
        if not artifacts[key]["present"]:
            raise ManifestError(
                "manifest_incomplete", f"manifest: hard-required artifact {key} absent (§11)"
            )
    ...
    if reports["present"]:
        if len(reports["items"]) != len(patches["items"]):
            raise ManifestError(
                "manifest_incomplete",
                "manifest: apply_reports / revision_patches length mismatch (§11)",
            )
    ...
    if (
        letter_entry["present"]
        and roadmap_entry["origin_date"] is not None
        and letter_entry["origin_date"] is not None
    ):
...
def compute_apply_chain_witness(manifest: dict, report_payloads):
    ...
    for i, report in enumerate(report_payloads):
        ...
        if not isinstance(digest, str) or not _SHA256_RE.match(digest):
            raise ManifestError(
                "manifest_incomplete", f"apply report [{i}]: format 1.3 requires patch_digest"
            )
...
def load_author_adjudication(
    ...
    *,
    ...
    if (
        not isinstance(payload["revision_round"], int)
        or isinstance(payload["revision_round"], bool)
        or payload["revision_round"] < 1
    ):
    ...
    for index, event in enumerate(events):
        ...
        if not isinstance(event["input_sha256"], str) or not _SHA256_RE.match(
            event["input_sha256"]
        ):
    ...
    for index, record in enumerate(records):
        ...
        try:
            ...
            _validate_author_targets(
                validator, record["authorized_targets"], "$.authorized_targets"
            )
            _validate_claim_authorizations(
                validator,
    ...
    for index, record in enumerate(collateral):
        ...
        if (
            record["authorizing_item_id"] not in by_item
            or record["constrained_item_id"] not in by_item
        ):
...
def derive_decision(
    ...
    base = None
    rule = None
    if any(v == "MADE_WORSE" and sev == "critical" for v, sev, _m in p1_items) or (
        "critical" in regression_severities
    ):
        base, rule = "Major Revision", "B1"
    elif (
        p1_count > 0
        and 2 * sum(1 for v, _s, _m in p1_items if v in ("NOT_ADDRESSED", "MADE_WORSE")) >= p1_count
    ):
        base, rule = "Major Revision", "B2"
    elif any(v in ("NOT_ADDRESSED", "MADE_WORSE", "CANNOT_VERIFY") for v, _s, _m in p1_items) or (
        "major" in regression_severities
    ):
        base, rule = "Major Revision", "B3"
    elif any(v == "PARTIALLY_ADDRESSED" and m == "must_fix" for v, _s, m in p1_items) or (
        "must_fix" in p2_partial_magnitudes
    ):
...
def _adjustment_chains(traceability, fails: Failures):
    ...
    for item_id, records in by_item.items():
        ...
        for rec in records:
            ...
            if prev is None:
                ...
                if prev_rec is None or prev_rec["item_id"] != item_id:
                    fails.add(
                        f"adjustment {rec['adjustment_id']}: supersedes_adjustment_id {prev!r} does not resolve on item {item_id}"
                    )
                    return {}
        ...
        if len(heads) != 1:
            fails.add(
                f"adjustment chain on {item_id}: exactly one head required, found {len(heads)}"
            )
            return {}
        chain = [heads[0]]
        ...
        if len(chain) != len(records):
            fails.add(
                f"adjustment chain on {item_id}: records outside the linear chain (cycle or fork)"
            )
            return {}
...
def check(
    ...
    must_fix_order = [
        item["id"] for item in roadmap_items if item["obligation_class"] == "must_fix"
    ]
    p12_ids = {
        item["id"]
        for item in roadmap_items
        if item["obligation_class"] in ("must_fix", "should_fix")
    }
    ...
    letter_sound = letter_blocks is not None and letter_ordinals_sound(
        letter_blocks, len(must_fix_order)
    )
    ...
    for item_id, rec in pre_by_item.items():
        ...
        if rec["obligation_class"] != item["obligation_class"]:
            fails.add(
                f"precommitment {item_id}: obligation_class {rec['obligation_class']} != roadmap {item['obligation_class']}"
            )
        if rec["inherited_criterion"]["roadmap_text"] != item["verification_criteria"]:
            fails.add(
                f"precommitment {item_id}: inherited_criterion.roadmap_text does not match the roadmap verbatim (§4)"
            )
        ...
        if item["obligation_class"] == "must_fix":
            ...
            if block_text is not None:
                if not has_letter_fields:
                    fails.add(
                        f"precommitment {item_id}: letter carries block {derived_ref} — letter_text/letter_item_ref REQUIRED (§5.1)"
                    )
                else:
                    ...
                    if rec["inherited_criterion"]["letter_text"] != block_text:
                        fails.add(
                            f"precommitment {item_id}: letter_text does not match the letter's Acceptance criteria byte-for-byte (§13)"
                        )
            elif has_letter_fields:
                fails.add(
                    f"precommitment {item_id}: letter layer absent at this item — letter fields must be absent (§5.1/§13)"
                )
        elif has_letter_fields:
    ...
    for rec in verdict_record["dissents"]:
        ...
        if pre is None:
            fails.add(
                f"dissent {rec['dissent_id']}: item {rec['item_id']} has no pre-commitment record"
            )
        elif rec["criterion_hash"] != canonical_hash(pre):
            fails.add(
                f"dissent {rec['dissent_id']}: criterion_hash does not recompute from the Phase-1 record (§7)"
            )
    ...
    for item_id, rec in verdict_by_item.items():
        ...
        if item["obligation_class"] == "consider":
            if applied != "not_precommitted":
                fails.add(
                    f"verdict {item_id}: consider items carry applied_criterion not_precommitted (§5.2)"
                )
        else:
            if applied == "not_precommitted":
                fails.add(
                    f"verdict {item_id}: not_precommitted is valid ONLY for consider items (§5.2)"
                )
                ...
                if dissent is None or dissent["item_id"] != item_id:
                    fails.add(
                        f"verdict {item_id}: applied_criterion {applied!r} does not resolve to a dissent on this item"
                    )
    ...
    for rec in verdict_record["escalation_exceptions"]:
        ...
        if ref is not None:
            ...
            if target is None:
                fails.add(
                    f"escalation exception {rec['exception_id']}: new_standard_ref {ref!r} does not resolve"
                )
            elif target["classification"] != "escalation_requested":
                fails.add(
                    f"escalation exception {rec['exception_id']}: new_standard_ref target is not escalation_requested (§5.1)"
                )

    # --- new-issue freeze witness (§13.5) ---
    if traceability["new_issues"] != verdict_record["new_issues"]:
        fails.add(
            "new_issues: §5.3 set is not WHOLE-RECORD byte-identical to the frozen §5.2 set (§3.3 freeze witness)"
        )
    new_issue_by_id = {}
    for rec in verdict_record["new_issues"]:
        ...
        if nearest is not None and nearest not in roadmap_by_id:
            fails.add(
                f"new issue {rec['new_issue_id']}: nearest_roadmap_item {nearest!r} does not resolve"
            )
    ...
    for item_id, row in row_by_item.items():
        ...
        if ROW_OBLIGATION_MAP[row["obligation_class"]] != item["obligation_class"]:
            fails.add(
                f"row {item_id}: obligation_class {row['obligation_class']} does not match the roadmap ({item['obligation_class']})"
            )
        ...
        if "cross_model_verdict" in row:
            ...
            if row["cross_model_status"] != derived:
                fails.add(
                    f"row {item_id}: cross_model_status {row['cross_model_status']!r} != per-emission derivation {derived!r} (§5.3)"
                )
    ...
    for item_id, row in row_by_item.items():
        ...
        tail_rebuttal = (
            bool(chain)
            and chain[-1]["basis"] == "valid_rebuttal"
            and chain[-1]["to_verdict"] == "FULLY_ADDRESSED"
        )
        if bool(row.get("addressed_by_rebuttal")) != tail_rebuttal:
            fails.add(
                f"row {item_id}: addressed_by_rebuttal marker must reflect a valid_rebuttal chain-tail upgrade to FULLY_ADDRESSED (§3.4)"
            )
    ...
    for rec in traceability["resolution_intents"]:
        ...
        if rec["answered_by"] == "system":
            ...
            if (
                intent_row is None
                or intent_row["obligation_class"] != "MUST_FIX"
                or "cross_model_verdict" not in intent_row
            ):
                fails.add(
    ...
    for rec in traceability["dissent_adjudications"]:
        ...
        if rec["adjudicator"] == "cross_model" and not cross_model_active:
            fails.add(
                f"dissent adjudication {rec['dissent_id']}: cross_model adjudicator on an inactive configuration (§11)"
            )
    ...
    for rec in traceability["reapplications"]:
        ...
        if prev is not None:
            ...
            if prev_rec is None or prev_rec["item_id"] != rec["item_id"]:
                fails.add(
                    f"reapplication {rec['reapplication_id']}: supersedes_reapplication_id {prev!r} does not resolve on the same item"
                )
    ...
    current_reapps = [
        rec
        for rec in traceability["reapplications"]
        if rec["reapplication_id"] not in superseded_reapps
    ]
    ...
    for rec in traceability["reapplications"]:
        ...
        for ref in rec["answer_refs"]:
            ...
            if kind == "adjudication":
                ...
                if adjudication is None or dissent is None:
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: answer ref {ref!r} does not resolve"
                    )
                    continue
                if adjudication["outcome"] != "original_upheld":
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: adjudication ref {ref!r} must resolve to original_upheld (§6)"
                    )
                if dissent["item_id"] != item_id:
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: adjudication ref {ref!r} is on a different item"
                    )
            else:
                ...
                if intent is None:
                    ...
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: intent ref {ref!r} is on a different item"
                    )
        ...
        if has_intent_ref and not has_adjudication_ref:
            ...
            if (
                row is None
                or row["obligation_class"] != "MUST_FIX"
                or "cross_model_verdict" not in row
            ):
                fails.add(
        ...
        changing = (
            rec["reapplied_verdict"] != "CANNOT_VERIFY"
            and rec["reapplied_verdict"] != rec["pre_reapplication_verdict"]
        )
        if rec["reapplied_verdict"] == "CANNOT_VERIFY":
            if derived:
                fails.add(
                    f"reapplication {rec['reapplication_id']}: CANNOT_VERIFY appends NOTHING mechanically (§6)"
                )
        elif changing:
            if len(derived) != 1:
                fails.add(
                    f"reapplication {rec['reapplication_id']}: a verdict-changing reapplication requires exactly one derived adjustment (§13)"
                )
            else:
                adj = derived[0]
                if adj["item_id"] != item_id or adj["basis"] != "cross_model_adjudication":
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: derived adjustment must be a cross_model_adjudication on the same item"
                    )
                if (
                    adj["from_verdict"] != rec["pre_reapplication_verdict"]
                    or adj["to_verdict"] != rec["reapplied_verdict"]
                ):
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: derived adjustment endpoints must bind to the RECORDED pre-value (§6)"
                    )
                if adj["rationale"] != rec["rationale"]:
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: derived adjustment rationale must be MECHANICALLY COPIED (§6)"
                    )
                if adj.get("residual_gap") != rec.get("residual_gap"):
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: derived adjustment residual_gap must be MECHANICALLY COPIED (§6)"
                    )
                if adj.get("evidence_anchor") != rec.get("evidence_anchor"):
                    fails.add(
                        f"reapplication {rec['reapplication_id']}: derived adjustment anchors must be MECHANICALLY COPIED, tags included (§5.3)"
                    )
        else:
            if derived:
                fails.add(
                    f"reapplication {rec['reapplication_id']}: no verdict change, so no derived adjustment may exist (§6)"
                )
    ...
    for rec in traceability["cross_model_resolutions"]:
        if rec["reapplication_id"] in resolutions_by_reapp:
            fails.add(
                f"reapplication {rec['reapplication_id']}: referenced by more than one CrossModelResolution"
            )
    ...
    for rec in traceability["cross_model_resolutions"]:
        ...
        if intent is None or intent["item_id"] != rec["item_id"]:
            fails.add(
                f"resolution {rec['resolution_id']}: intent_id does not resolve on the same item"
            )
        if reapp is None or reapp["item_id"] != rec["item_id"]:
            fails.add(
                f"resolution {rec['resolution_id']}: reapplication_id does not resolve on the same item"
            )
            continue
        ...
        if rec["resolved_by"] == "system":
            if reapp["reapplied_verdict"] == "CANNOT_VERIFY":
                fails.add(
                    f"resolution {rec['resolution_id']}: a CANNOT_VERIFY reapplication derives no system resolution (§5.3)"
                )
            else:
                expected_state = (
                    "primary_upheld"
                    if reapp["reapplied_verdict"] == reapp["pre_reapplication_verdict"]
                    else "primary_revised"
                )
                if rec["state"] != expected_state:
                    fails.add(
                        f"resolution {rec['resolution_id']}: state {rec['state']!r} != §9 derivation ({expected_state!r})"
                    )
        else:
            if reapp["reapplied_verdict"] != "CANNOT_VERIFY":
                fails.add(
                    f"resolution {rec['resolution_id']}: the user acceptance form requires a CANNOT_VERIFY reapplication (§5.3)"
                )
    ...
    for rec in traceability["g2d_acceptances"]:
        ...
        if reapp is None or reapp["item_id"] != rec["item_id"]:
            fails.add(
                f"acceptance {rec['acceptance_id']}: reapplication_id does not resolve on the same item"
            )
        elif reapp["reapplied_verdict"] != "CANNOT_VERIFY":
            fails.add(
                f"acceptance {rec['acceptance_id']}: the accepted reapplication must be CANNOT_VERIFY (§6 G2(d))"
            )
        ...
        if row is not None and row["final_verdict"] != "CANNOT_VERIFY":
            fails.add(
                f"acceptance {rec['acceptance_id']}: the accepted item's final_verdict must be CANNOT_VERIFY (§6 G2(d))"
            )
        backing = [
            adj
            for adj in traceability["adjustments"]
    ...
    for adj in traceability["adjustments"]:
        if adj["basis"] == "user_accepted_fail_closed":
            ...
            if acceptance is None or acceptance["item_id"] != adj["item_id"]:
                fails.add(
                    f"adjustment {adj['adjustment_id']}: acceptance ref does not resolve on the same item"
                )
                continue
            acceptance_adj_by_reapp[acceptance["reapplication_id"]] = adj
            reapp = reapp_by_id.get(acceptance["reapplication_id"])
            if reapp is not None and reapp.get("cannot_verify_reason") != adj.get(
                "cannot_verify_reason"
            ):
                fails.add(
                    f"adjustment {adj['adjustment_id']}: cannot_verify_reason must be copied from the accepted reapplication (§3.4)"
                )
    ...
    for rec in traceability["rebuttal_adjudications"]:
        ...
        if not cross_model_active:
            fails.add(
                f"rebuttal adjudication {rec['rebuttal_adjudication_id']}: the §3.4 pass exists only under active cross-model (§11)"
            )
    ...
    for adj in traceability["adjustments"]:
        ...
        if has_check:
            ...
            match = (
                _CHECK_ADJUDICATED_RE.match(check_value) if isinstance(check_value, str) else None
            )
            if match:
                radj = radj_by_id.get(match.group(1))
                if radj is None or radj["verdict"] != "upheld" or radj["item_id"] != adj["item_id"]:
                    fails.add(
                        f"adjustment {adj['adjustment_id']}: adjudicated:* must resolve to an upheld RebuttalAdjudication on the same item (§3.4)"
                    )
            elif check_value == "single_family_disclosed" and cross_model_active:
                fails.add(
                    f"adjustment {adj['adjustment_id']}: single_family_disclosed is invalid on an active cross-model run (§3.4)"
                )
    ...
    for rec in traceability["pending_rebuttal_upgrades"]:
        ...
        if rec["drafted_adjustment"]["item_id"] != rec["item_id"]:
            fails.add(
                f"proposal {rec['proposal_id']}: drafted_adjustment.item_id must equal the proposal's item_id (§5.3)"
            )
        ...
        if kind == "challenged":
            challenged_bodies.append(
                (rec["proposal_id"], rec["item_id"], rec["drafted_adjustment"])
            )
        if kind == "challenged":
            radj = radj_by_id.get(ref_id)
            if radj is None or radj["item_id"] != rec["item_id"]:
                fails.add(
                    f"proposal {rec['proposal_id']}: challenged ref does not resolve on the same item"
                )
            elif radj["verdict"] != "challenged":
                fails.add(
                    f"proposal {rec['proposal_id']}: challenged disposition must reference a challenged adjudication"
                )
        else:
            adj = adjustments_by_id.get(ref_id)
            if adj is None or adj["item_id"] != rec["item_id"] or adj["basis"] != "valid_rebuttal":
                fails.add(
                    f"proposal {rec['proposal_id']}: booked ref must resolve to a valid_rebuttal adjustment on the same item"
                )
                continue
            if ref_id in booked_by_adjustment:
                fails.add(
                    f"proposal {rec['proposal_id']}: adjustment {ref_id} already booked by {booked_by_adjustment[ref_id]}"
                )
            ...
            booked = {
                ...
                if key
                not in ("adjustment_id", "supersedes_adjustment_id", "critical_rebuttal_check")
            }
            if drafted != booked:
                fails.add(
            ...
            if kind == "booked":
                match = (
                    _CHECK_ADJUDICATED_RE.match(check_value)
                    if isinstance(check_value, str)
                    else None
                )
                if not match or radj_by_id.get(match.group(1), {}).get("verdict") != "upheld":
                    fails.add(
                        f"proposal {rec['proposal_id']}: booked disposition requires an upheld adjudicated:* check (§3.4)"
                    )
            elif kind == "booked_disclosed":
                if check_value != "pass_unavailable_disclosed":
                    fails.add(
                        f"proposal {rec['proposal_id']}: booked_disclosed requires critical_rebuttal_check pass_unavailable_disclosed"
                    )
                if not cross_model_active:
                    fails.add(
                        f"proposal {rec['proposal_id']}: booked_disclosed exists only under active cross-model (§11)"
                    )
            elif kind == "booked_single_family":
                if check_value != "single_family_disclosed":
                    fails.add(
                        f"proposal {rec['proposal_id']}: booked_single_family requires critical_rebuttal_check single_family_disclosed"
                    )
                if cross_model_active:
                    fails.add(
                        f"proposal {rec['proposal_id']}: booked_single_family is invalid under active cross-model (§3.4)"
                    )
    for adj in traceability["adjustments"]:
        if (
            adj["basis"] == "valid_rebuttal"
            and _is_critical(adj["item_id"])
            and adj["adjustment_id"] not in booked_by_adjustment
        ):
            fails.add(
    ...
    if not cross_model_active:
        for item_id, row in row_by_item.items():
            if row.get("cross_model_status") in ("agree", "diverges", "unavailable"):
                fails.add(
                    f"row {item_id}: cross_model_status {row['cross_model_status']!r} implies an active configuration (§11)"
                )
    ...
    for item_id in must_fix_order:
        ...
        if (
            ...
            and any(
                ...
                for field in ("evidence_anchor", "confidence", "competence_basis")
            )
        ):
            fails.add(
    ...
    if di["per_item"] != expected_per_item:
        fails.add(
            "decision_inputs.per_item does not equal the recomputed per-must_fix operand list (immutable roadmap order, §13)"
        )

    expected_counts = {prio: {verdict: 0 for verdict in VERDICTS} for prio in OBLIGATION_CLASSES}
    expected_magnitudes = {
        prio: {mag: 0 for mag in RESIDUAL_OBLIGATION_CLASSES} for prio in OBLIGATION_CLASSES
    }
    for item_id, row in row_by_item.items():
        item = roadmap_by_id.get(item_id)
        if item is None:
    ...
    if di["verdict_counts"] != expected_counts:
        fails.add(
            "decision_inputs.verdict_counts does not recompute from the final row verdicts (§13)"
        )
    if di["residual_obligation_class_counts"] != expected_magnitudes:
        fails.add(
            "decision_inputs.residual_obligation_class_counts does not recompute from the PARTIALLY_ADDRESSED rows (§13)"
        )

    p2_ids = [item["id"] for item in roadmap_items if item["obligation_class"] == "should_fix"]
    expected_num = sum(
        1
        for item_id in p2_ids
        if row_by_item.get(item_id, {}).get("final_verdict")
        in ("FULLY_ADDRESSED", "PARTIALLY_ADDRESSED")
    )
    if di["should_fix_addressed_rate"] != {"numerator": expected_num, "denominator": len(p2_ids)}:
        fails.add(
            "decision_inputs.should_fix_addressed_rate does not recompute over final should_fix verdicts (§6)"
        )

    expected_regressions = [
        {"new_issue_id": rec["new_issue_id"], "severity": rec["severity"]}
        for rec in verdict_record["new_issues"]
        if rec["attribution"] == "regression"
    ]
    ...
    if di["regressions"] != expected_regressions:
        fails.add(
            "decision_inputs.regressions does not equal the regression-attributed frozen new issues (§8/§13)"
        )
    if di["non_regression_new_issue_ids"] != expected_non_regression:
        fails.add(
            "decision_inputs.non_regression_new_issue_ids does not equal the frozen previously_missed/indeterminate ids (§8/§13)"
        )
    ...
    if di["escalations"] != expected_escalations:
        fails.add(
            "decision_inputs.escalations does not equal the joined exception/approval summary (§5.3)"
        )

    if di["apply_chain_witness"] != witness:
        fails.add(
            f"decision_inputs.apply_chain_witness {di['apply_chain_witness']!r} != recomputed witness {witness!r} (§11)"
        )
    ...
    if bound_tripped:
        for dissent_id in dissent_by_id:
            if dissent_id not in adjudications_by_dissent:
                pending.append(
                    f"G2(a): dissent {dissent_id} lacks its DissentAdjudication (bound tripped — covering cardinality)"
                )
    ...
    def _covering_resolution(item_id, final_verdict):
        for rec in traceability["cross_model_resolutions"]:
            ...
            if reapp is None:
                continue
            anchor = (
                reapp["reapplied_verdict"]
                if rec["resolved_by"] == "system"
                else reapp["pre_reapplication_verdict"]
            )
    ...
    for item_id, row in row_by_item.items():
        if (
            row.get("cross_model_status") == "diverges"
            and ROW_OBLIGATION_MAP[row["obligation_class"]] == "must_fix"
        ):
            if not _covering_resolution(item_id, row["final_verdict"]):
                pending.append(
                    f"G2(b): must_fix diverges row {item_id} has no covering CrossModelResolution"
                )
    for rec in verdict_record["escalation_exceptions"]:
        if rec["exception_id"] not in approvals_by_exception:
            pending.append(
                f"G2(c): escalation exception {rec['exception_id']} is pending user approval"
            )
    ...
    for dissent_id, adjudication in adjudications_by_dissent.items():
        ...
        current = holders[0]
        if (
            current["reapplied_verdict"] == "CANNOT_VERIFY"
            and current["reapplication_id"] not in accepted_reapps
        ):
            pending.append(
                f"G2(d): original_upheld dissent {dissent_id} fail-closed reapplication awaits user resolution"
            )
    ...
    if emitted_state == "aborted":
        ...
        if "reject_recommended" in di:
            fails.add(
                "decision_inputs.reject_recommended must be ABSENT on a gated emission (§5.3 presence biconditional)"
            )
        ...
        if g1 and reason != "criteria_drift":
            fails.add(
                "Step 1 recomputes G1 (silent verdict change) — the aborted emission's true root cause is criteria_drift (§13)"
            )
        if reason == "criteria_drift" and not g1:
            fails.add(
                "the emission claims [RE-REVIEW-ABORT: criteria_drift] but no silent verdict change recomputes (§13)"
            )
    elif g1:
        recomputed["expect"] = ("aborted", "criteria_drift")
        fails.add(
            "Step 1 recomputes G1 (silent verdict change) — the emission must be [RE-REVIEW-ABORT: criteria_drift]"
        )
    elif pending:
        ...
        if emitted_state == "user_review_required":
            ...
            for item_id in must_fix_order:
                ...
                p1_operands.append(
                    (row["final_verdict"], severity, _final_residual_obligation_class(item_id))
                )
            p2_partial_magnitudes = [
                _final_residual_obligation_class(item_id)
                for item_id in p2_ids
                if row_by_item.get(item_id, {}).get("final_verdict") == "PARTIALLY_ADDRESSED"
            ]
            p2_made_worse = any(
                row_by_item.get(item_id, {}).get("final_verdict") == "MADE_WORSE"
                for item_id in p2_ids
            )
            ...
            di_p1 = [
                (
                    entry["final_verdict"],
                    entry["driving_severity"],
                    entry.get("residual_obligation_class"),
                )
                for entry in di["per_item"]
            ]
            di_p2_partials = [
                mag
                for mag in RESIDUAL_OBLIGATION_CLASSES
            ...
            if emitted_state != raw_decision:
                fails.add(
                    f"decision_state {emitted_state!r} != Steps 2-3 recomputed from the raw records ({raw_decision!r}, base {raw_rule})"
                )
            if di_decision != raw_decision:
                fails.add(
                    f"Steps 2-3 from DecisionInputs ({di_decision!r}) diverge from the raw-record recomputation ({raw_decision!r})"
                )
            if "reject_recommended" not in di:
                fails.add(
                    "decision_inputs.reject_recommended must be PRESENT on a non-gated emission (§5.3 presence biconditional)"
                )
            else:
                if di["reject_recommended"] != raw_reject:
                    fails.add(
                        f"reject_recommended {di['reject_recommended']!r} != recomputed {raw_reject!r} (§6)"
                    )
                if di_reject != raw_reject:
                    fails.add(
                        "reject_recommended recomputed from DecisionInputs diverges from the raw-record recomputation"
                    )
...
def _verify_file_hash(path: Path, expected_sha256: str, label: str) -> bytes:
    ...
    if actual != expected_sha256:
        raise ManifestError(
            "manifest_hash_mismatch", f"{label}: file bytes do not match the manifest sha256 (§11)"
        )
    return raw
...
def run(argv=None) -> int:
    ...
    try:
        ...
        roadmap_raw = _verify_file_hash(
            args.roadmap, artifacts["revision_roadmap"]["sha256"], "revision_roadmap file"
        )
        try:
        ...
        if artifacts["editorial_decision_letter"]["present"]:
            if args.letter is None:
                raise ManifestError(
                    "manifest_incomplete",
                    "manifest declares editorial_decision_letter present — --letter is required",
                )
            letter_raw = _verify_file_hash(
                args.letter,
                artifacts["editorial_decision_letter"]["sha256"],
                "editorial_decision_letter file",
            )
            letter_blocks = parse_letter_blocks(letter_raw.decode("utf-8"))
        elif args.letter is not None:
            raise ManifestError(
                "manifest_incomplete",
                "--letter provided but the manifest declares the letter absent",
            )

        report_entries = (
            artifacts["apply_reports"]["items"] if artifacts["apply_reports"]["present"] else []
        )
        ...
        if precommitment["input_manifest_hash"] != canonical_hash(manifest):
            raise ManifestError(
                "manifest_hash_mismatch",
                "precommitment.input_manifest_hash does not recompute from the manifest (§11 chain)",
            )
        if verdict_record["precommitment_hash"] != canonical_hash(precommitment):
            raise ManifestError(
                "manifest_hash_mismatch",
                "verdict_record.precommitment_hash does not recompute from the precommitment artifact (§11 chain)",
            )
        if traceability["verdict_record_hash"] != canonical_hash(verdict_record):
            raise ManifestError(
                "manifest_hash_mismatch",
                "traceability.verdict_record_hash does not recompute from the verdict_record artifact (§11 chain)",
            )
