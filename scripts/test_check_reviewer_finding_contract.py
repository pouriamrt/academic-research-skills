...
def test_m2_finding_contract_body_gutted(tmp_path):
    ...
    _edit(
        ...
        SCORING_AGENTS[0],
        lambda t: t.replace(
            "Do not manufacture findings to fill a quota; do not omit real ones to seem agreeable.",
            "Use your judgment.",
        ),
    )
    code, err = _run2(root)
...
def test_m4_coverage_receipt_skeleton_dropped(tmp_path):
    ...
    _edit(
        ...
        SCORING_AGENTS[3],
        lambda t: t.replace(
            "### Coverage Receipt (only when Strengths or Weaknesses is empty)", "### Notes"
        ),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m5_every_scoring_agent_is_pinned(tmp_path):
    ...
    for rel in SCORING_AGENTS:
        ...
        _edit(
            ...
            rel,
            lambda t: t.replace(
                "**Finding Contract (#574 A1/A2/A3)** — governs", "**Notes** — governs"
            ),
        )
...
def test_m6_template_quota_restored(tmp_path):
    ...
    _edit(
        ...
        TEMPLATE_REL,
        lambda t: t.replace(
            "List every weakness you actually found",
...
def test_m8_template_absence_row_removed(tmp_path):
    ...
    def drop_absence_row(t: str) -> str:
        return "\n".join(ln for ln in t.splitlines() if not ln.startswith("| `absence` |")) + "\n"
...
def test_m9_template_must_anchor_weakened(tmp_path):
    ...
    _edit(
        ...
        TEMPLATE_REL,
        lambda t: t.replace(
            "**Critical/Major findings MUST carry an adequate anchor",
...
def test_m11_schema6_canonical_sentence_removed(tmp_path):
    ...
    _edit(
        ...
        SCHEMAS_REL,
        lambda t: t.replace(
            "the CANONICAL single source for finding severity across the reviewer stack (#574 A3)",
            "a severity value",
        ),
    )
    code, err = _run2(root)
...
def test_m12_synth_severity_fallback_tag_removed(tmp_path):
    root = _mirror(tmp_path)
    _edit(root, SYNTH_REL, lambda t: t.replace("[SEVERITY-SOURCE: letter-fallback]", "a note"))
    code, err = _run2(root)
    assert code == 1
...
def test_m13_da_band_consistency_removed(tmp_path):
    ...
    _edit(
        ...
        DA_REL,
        lambda t: t.replace(
            "the same defect class with the same decision impact lands in the same band every time",
...
def test_m14_standards_severity_column_restored(tmp_path):
    ...
    _edit(
        ...
        STANDARDS_REL,
        lambda t: t.replace(
            "| Situation | Decision impact |", "| Situation | Severity | Decision impact |"
        ),
    )
    code, err = _run2(root)
    assert code == 1
    assert "severity-assignment column regression" in err or "decision-impact table header" in err
...
def test_m16_standards_b1_principle_gutted(tmp_path):
    ...
    _edit(
        root,
        STANDARDS_REL,
        lambda t: t.replace("**Register is independent of severity.**", "Politeness matters."),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m17_standards_base_rate_restored(tmp_path):
    ...
    _edit(
        ...
        STANDARDS_REL,
        lambda t: t.replace(
            "the decision follows the criteria, never a frequency "
    ...
    code, err = _run2(root)
    assert code == 1
    assert "base-rate anchor regression" in err or "frequency-free first-pass acceptance" in err
...
def test_m22_da_anchor_list_partially_edited(tmp_path):
    ...
    _edit(
        ...
        lambda t: t.replace(
            ...
            "`text` / `table` / `figure` / `equation` / `dataset`",
            1,
        ),
    )
    code, err = _run2(root)
...
def test_m23_schema6_confidence_row_deleted(tmp_path):
    ...
    def drop_row(t: str) -> str:
        return (
            "\n".join(
                ln for ln in t.splitlines() if not ln.startswith("| `confidence` | integer |")
            )
            + "\n"
        )
...
def test_m26_skill_fake_diversity_restored(tmp_path):
    ...
    _edit(
        ...
        SKILL_REL,
        lambda t: t.replace(
            "**Overlap suppression**", "**Duplicate criticisms across reviewers**"
        ).replace(
            "unexecutable under blindness (Iron Rule #2) and destroys the corroboration signal",
            "R1/R2/R3 raise identical points = fake diversity",
        ),
    )
    code, err = _run2(root)
    assert code == 1
    assert "overlap-prohibition regression" in err or "overlap-suppression anti-pattern row" in err
...
def test_m27_synth_mandatory_merit_restored(tmp_path):
    ...
    _edit(
        ...
        SYNTH_REL,
        lambda t: t.replace(
            "Point out genuine merits where the reviewer cards found them — "
...
def test_m28_da_critical_band_reverted(tmp_path):
    ...
    _edit(
        ...
        DA_REL,
        lambda t: t.replace(
            "Fatal flaw in core argument or methodology that blocks acceptance "
    ...
    code, err = _run2(root)
    assert code == 1
    assert "DA CRITICAL decision-impact definition" in err or "unfixable-only definition" in err
...
def test_m32_da_header_superset_rename(tmp_path):
    ...
    def rename(t: str) -> str:
        ...
        tail = tail.replace(
            "| Evidence Anchor | Confidence |", "| Evidence Anchor Note | Confidence Basis |", 1
        )
        return head + tail
...
def test_m33_synth_split_prior_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "directional split prior regression" in err or "divergence-weighting edge case" in err
...
def test_m34_decision_template_conservative_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "asymmetric arbitration regression" in err or "arbitration rationale vocabulary" in err
...
def test_m35_guided_mandatory_opener_restored(tmp_path):
    ...
    assert code == 1
    assert (
        "mandatory-strength opener regression" in err
...
def test_m36_rqt_minimum_strength_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "minimum-strength check regression" in err or "review-quality balance check" in err
...
def test_m37_domain_advisory_disposition_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "off-enum severity disposition regression" in err or "field-norm enum disposition" in err
...
def test_m38_strength_anchor_bullet_gutted(tmp_path):
    ...
    _edit(
        ...
        SCORING_AGENTS[1],
        lambda t: t.replace(
            "- Every strength carries a typed Evidence Anchor too "
...
def test_m39_response_template_bounded_again(tmp_path):
    ...
    _edit(
        root,
        RESPONSE_TEMPLATE_REL,
        lambda t: t.replace("W1..Wn (every emitted weakness, unbounded — #574 A1)", "W1-W5"),
    )
    code, err = _run2(root)
    assert code == 1
    assert "bounded W-range mirror regression" in err or "unbounded W1..Wn witness" in err
...
def test_m43_perspective_assumption_quota_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "assumption-audit quota regression" in err or "conditional assumption-audit gate" in err
...
def test_m44_rqt_base_rate_restored(tmp_path):
    ...
    assert code == 1
    assert (
        "author-population base-rate regression" in err
...
def test_m47_single_mirror_rebounded(tmp_path):
    ...
    def rebound_r3(t: str) -> str:
        ...
        head, tail = t[:idx], t[idx:]
        tail = tail.replace("W1..Wn (every emitted weakness, unbounded — #574 A1)", "W1–W4", 1)
        return head + tail
...
def test_m48_stats_triage_note_removed(tmp_path):
    ...
    _edit(
        ...
        STATS_REL,
        lambda t: t.replace(
            "**Triage levels, not finding severities (#574 A3).**", "**Severity legend.**"
        ),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m49_synth_lone_major_escalation_restored(tmp_path):
    ...
    assert code == 1
    assert (
        "lone-Major arbitration rule" in err
...
def test_m51_schema6_strengths_destructured(tmp_path):
    ...
    _edit(
        ...
        SCHEMAS_REL,
        lambda t: t.replace(
            "Strength objects `{description: string, evidence_anchor: object}`", "bare strings"
        ),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m55_wide_bounded_range_caught(tmp_path):
    ...
    _edit(
        root,
        RESPONSE_TEMPLATE_REL,
        lambda t: t.replace("W1..Wn (every emitted weakness, unbounded — #574 A1)", "W1-W10", 1),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m56_schema6_row_migrated_to_schema7(tmp_path):
    ...
    def migrate(t: str) -> str:
        row = next(ln for ln in t.splitlines() if ln.startswith("| `confidence` | integer |"))
        t = t.replace(row + "\n", "")
        return t.replace(
...
def test_m60_subclaim_severity_inheritance_removed(tmp_path):
    ...
    _edit(
        ...
        SYNTH_REL,
        lambda t: t.replace(
            "All sub-claims decomposed from one parent share the parent's transported severity",
...
def test_m61_cross_model_da_quota_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "cross-model DA fixed-count regression" in err or "cross-model DA no-quota prompt" in err
...
def test_m63_skill_unconditional_veto_restored(tmp_path):
    ...
    assert code == 1
    assert (
        "unconditional DA-CRITICAL veto regression" in err or "adjudication wording missing" in err
    )
...
def test_m64_da_delivered_contract_weakened(tmp_path):
    ...
    _edit(
        ...
        DA_REL,
        lambda t: t.replace(
            "CRITICAL/MAJOR require an adequate, applicable one", "CRITICAL/MAJOR may use one", 1
        ),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m68_schema7_severity_field_deleted(tmp_path):
    ...
    def drop(t: str) -> str:
        return (
            "\n".join(
                ln for ln in t.splitlines() if "Transported Schema 6 finding severity" not in ln
            )
            + "\n"
        )
...
def test_m70_standards_qualitative_base_rate_restored(tmp_path):
    ...
    _edit(
        ...
        STANDARDS_REL,
        lambda t: t.replace(
            "never a frequency expectation", "uncommon in practice, but judged on the criteria"
        ),
    )
    code, err = _run2(root)
    assert code == 1
    assert (
        "qualitative base-rate cue regression" in err
...
def test_m71_schema7_anchor_row_deleted(tmp_path):
    ...
    def drop(t: str) -> str:
        ...
        for ln in t.splitlines():
            if ln.startswith("## Schema 7:"):
                in_s7 = True
            elif ln.startswith("## Schema 8:"):
                in_s7 = False
            if in_s7 and ln.startswith("| `evidence_anchor` | object |"):
                continue
            keep.append(ln)
        return "\n".join(keep) + "\n"
...
def test_m76_field_analyst_register_softening_restored(tmp_path):
    ...
    code, err = _run2(root)
    assert code == 1
    assert "register-softens-verdict regression" in err or "developmental-register rule" in err
...
def test_m77_schema7_semantics_rewritten(tmp_path):
    ...
    _edit(
        ...
        SCHEMAS_REL,
        lambda t: t.replace(
            "Fallback provenance for `confidence` — the verbatim tag",
...
def test_m29_calibration_bridge_removed(tmp_path):
    ...
    _edit(
        root,
        CALIBRATION_REL,
        lambda t: t.replace("measurement-reading prior for calibration", "prior").replace(
            "no verdict is shaded stricter on this prior's account", "verdicts may account for it"
        ),
    )
    code, err = _run2(root)
    assert code == 1
...
def test_m19_skill_overlap_prohibition_restored(tmp_path):
    ...
    _edit(
        ...
        SKILL_REL,
        lambda t: t.replace(
            "independent overlap in findings is legitimate corroboration", "no duplicate criticisms"
        ),
    )
    code, err = _run2(root)
    assert code == 1
    assert "overlap-corroboration standard" in err or "overlap-prohibition regression" in err
