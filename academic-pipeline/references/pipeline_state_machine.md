...
| checkpoint (post-1) | Stage 2 | User confirms + no experiment routing flags | handoff RQ Brief + Methodology Blueprint + Bibliography + Synthesis |
...
| checkpoint (post-1.5c) | Stage 2 | User confirms | handoff RQ Brief + Methodology Blueprint + Bibliography + Synthesis + Schema 10 + Schema 11 + Schema 12 |
...
1. After delivering the Process Record (MD + PDF, English-only), the orchestrator prompts for a terminal acknowledgement.
2. Terminal acknowledgement vocabulary: `finish` / `end` / `done` / `confirm`, or an unambiguous natural-language equivalent that accepts the deliverables. Change requests (content corrections, format changes) keep Stage 6 `in_progress` — they are not acknowledgements.
