...
def test_writer_missing_section_fails(writer_text):
    errs = check_writer(_mutate(writer_text, "## Patch-Document Revision Emission (#390)"))
...
def test_writer_lost_escalation_tag_fails(writer_text):
    errs = check_writer(_mutate(writer_text, "[PATCH-ESCALATION-REQUIRED: layer=pre_drafting"))
    assert errs and "escalation tag" in errs[0]


def test_writer_lost_sidecar_path_fails(writer_text):
    errs = check_writer(_mutate(writer_text, "phase6_*/revision_patch_round<N>.json"))
...
def test_orchestrator_lost_no_rewrite_window_fails(orchestrator_text):
    errs = check_orchestrator(
        _mutate(orchestrator_text, "nothing may rewrite the draft between steps 1 and 3")
    )
...
def test_skill_mode_row_regressed_to_full_draft_fails(skill_text):
    row = next(
        line for line in skill_text.splitlines() if line.lstrip().startswith("| `revision` |")
    )
    regressed = skill_text.replace(
        row, '| `revision` | "Revise paper" | 8->5->6 | Revised draft with tracked changes |'
    )
...
def test_protocol_lost_apply_command_fails(protocol_text):
    errs = check_protocol_doc(_mutate(protocol_text, "python scripts/ars_apply_revision_patch.py"))
...
def test_formatter_lost_ordering_fails(formatter_text, word_count_text):
    errs = check_marker_rules(
        _mutate(formatter_text, "ONLY AFTER every marker-dependent gate"), word_count_text
    )
...
def test_word_count_lost_strip_rule_fails(formatter_text, word_count_text):
    errs = check_marker_rules(
        formatter_text,
        _mutate(word_count_text, "Strip every `<!--...-->` comment before computing"),
    )
...
def test_threshold_lock_amendment_lost_decision_fails(spec_text, apply_src):
    errs = check_threshold_lock(_mutate(spec_text, "heading-anchor exemption"), apply_src)
...
def test_cli_default_regressed_to_none_fails(spec_text, apply_src):
    ...
    regressed = apply_src.replace("default=DEFAULT_TOUCHED_RATIO_THRESHOLD,", "default=None,")
