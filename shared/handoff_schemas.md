...
**Consumer**: academic-paper (revision mode, if further revision needed), pipeline orchestrator. Schema 18 is carried forward via Material Passport (Schema 9) for cross-stage audit.
...
- `residual_action`: What remains to be done if not fully addressed. This is a single concern-level string (one per Schema 18 row), distinct from the per-commitment `unfulfilled_rationale` field nested inside each `commitment_extracted` object below. Two coherence conventions govern how the two interact:
