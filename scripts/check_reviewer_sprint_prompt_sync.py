...
def canonical_digest(
    ...
    payload = (
        ...
        + b"\0fragments\0"
        + b"".join(
            name.encode("utf-8") + b"\0" + fragments.get(name, "").encode("utf-8") + b"\0"
...
def _expected_slot_table(errors: list[str]) -> tuple[str, ...]:
    ...
    for rel, config in ROLE_CONFIG.items():
        ...
        if config.get("phase2_insert") == "methodology-receipt":
            ...
            errors.append(f"{rel}: unknown Phase 2 insert fragment {config.get('phase2_insert')!r}")
        if config.get("extraction") == "methodology-extraction":
            phase2_label += " + extraction"
        elif "extraction" in config:
            errors.append(f"{rel}: unknown extraction fragment {config.get('extraction')!r}")
...
def _render(template: str, values: dict[str, str], label: str, errors: list[str]) -> str:
    ...
    rendered = SLOT_RE.sub(lambda match: values.get(match.group(1), match.group(0)), template)
    ...
    if DOUBLE_BRACE_RE.search(rendered):
        errors.append(
            f"{label}: canonical render left an unresolved or malformed double-brace placeholder"
        )
    return rendered
...
def check(root: Path) -> list[str]:
    ...
    if BACKLINK not in sprint_ref:
        errors.append(f"{SPRINT_REF_REL}: canonical prompt-source backlink missing ({BACKLINK})")
    ...
    for rel, config in ROLE_CONFIG.items():
        ...
        if runtime_phase1 != expected1:
            errors.append(f"{rel}: dispatcher-visible Phase 1 runtime extraction drift")
        if phase1 is None:
        ...
        if insert_name:
            template2 = _splice(template2, fragments.get(insert_name, ""), rel, errors)
        expected2 = _render(template2, config, rel, errors)
        if runtime_phase2 != expected2:
            errors.append(f"{rel}: dispatcher-visible Phase 2 runtime extraction drift")
        if phase2 is None:
        ...
        if not extraction_name and EXTRACTION_HEADING in text:
            ...
            # drift.
            errors.append(
                f"{rel}: carries {EXTRACTION_HEADING!r} but declares no extraction fragment"
            )
        if extraction_name:
            expected_extraction = _render(fragments.get(extraction_name, ""), config, rel, errors)
            ...
            if runtime_extraction != expected_extraction:
                errors.append(f"{rel}: dispatcher-visible extraction runtime extraction drift")
            if delivered_extraction is None:
                ...
                errors.append(f"{rel}: extraction mirror drift (byte-exact compare failed)")
    ...
    if synth_heading_count > 1:
        errors.append(f"{SYNTH_REL}: synthesizer protocol heading appears more than once")
