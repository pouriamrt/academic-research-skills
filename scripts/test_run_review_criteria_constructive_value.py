...
def _env(tmp_path: Path, fake_bin: Path) -> dict[str, str]:
    ...
    codex_home.mkdir()
    (codex_home / "auth.json").write_text('{"tokens":{"access":"fake"}}', encoding="utf-8")
...
def test_frozen_assets_and_exact_balanced_plan_validate() -> None:
    ...
    plan = json.loads(runner.CALL_PLAN_PATH.read_text(encoding="utf-8"))
...
def test_provider_response_schema_projection_keeps_local_validation_only_local() -> None:
    source = json.loads(runner.OUTPUT_SCHEMA_PATH.read_text(encoding="utf-8"))
    ...
    projected_property = runner._project_provider_response_schema(property_named_like_annotation)
...
def test_init_run_seals_same_context_bytes_and_only_treatment_binding(
    ...
    assert plan["tools"] == []
    baseline = (run_dir / "prompts/RCV-01-baseline-r1.txt").read_text(encoding="utf-8")
    treatment = (run_dir / "prompts/RCV-01-treatment-r1.txt").read_text(encoding="utf-8")
...
def test_detection_fails_closed_on_unknown_disable_feature(tmp_path) -> None:
    ...
    feature_names = tuple(name for name in runner.DISABLED_FEATURES if name != missing)
...
def test_fake_subscription_dispatch_is_contained_complete_and_idempotent(
    ...
    manifest = json.loads((run_dir / "execution-manifest.json").read_text(encoding="utf-8"))
    ...
    provider_schema = runner._project_provider_response_schema(
        json.loads(runner.OUTPUT_SCHEMA_PATH.read_text(encoding="utf-8"))
    )
    provider_schema_sha256 = hashlib.sha256(runner._json_bytes(provider_schema)).hexdigest()
    for row in rows:
...
def test_dispatch_fails_closed_on_tool_event_or_semantic_output(
    ...
    fake_bin, capture = _fake_codex(tmp_path, forbidden_event=forbidden, invalid_output=invalid)
    environ = _env(tmp_path, fake_bin)
    monkeypatch.setattr(runner, "_verify_frozen_commit", lambda _commit, _assets: None)
    run_dir = tmp_path / "run"
    runner.init_run(_init_args(run_dir))
    ...
    assert json.loads(blocked[0].read_text(encoding="utf-8"))["api_spend_usd"] == 0
    before = capture.read_bytes()
...
def test_blind_packet_two_experts_finalize_and_score(monkeypatch, tmp_path) -> None:
    ...
    for expert_id in ("expert-a", "expert-b"):
        ...
        path.write_text(json.dumps(_label_file(packet, packet_sha, expert_id)), encoding="utf-8")
        runner.validate_labels(argparse.Namespace(packet=packet_path, labels=path))
        expert_paths.append(path)
    decisions_path = tmp_path / "decisions.json"
    decisions_path.write_text(json.dumps(_decisions(packet, packet_sha)), encoding="utf-8")
    ...
    report = runner._measurement_report_value(
        ...
        paired_ref=(
            "evals/heldout/review_criteria_constructive_value/runs/fixture/paired-adjudication.json"
        ),
        paired_sha256="a" * 64,
        execution_ref=(
            "evals/heldout/review_criteria_constructive_value/runs/fixture/execution-manifest.json"
        ),
...
def test_forged_arm_map_is_rejected(monkeypatch, tmp_path) -> None:
    ...
    decisions_path = tmp_path / "decisions.json"
    decisions_path.write_text(json.dumps(_decisions(packet, packet_sha)), encoding="utf-8")
    arm_map = json.loads(map_path.read_text(encoding="utf-8"))
