...
import check_heldout_measurement_report as checker
import score_calibration_run as scorer
from _e4_evidence import sha256_file
from dispatch_calibration_panel import (
    MANIFEST_NAME,
    PreconditionFailure,
...
def bind_metrics(metrics: dict, work: Path, identity: dict, args) -> None:
    ...
    for key, row in scored.items():
        if row.get("attempt_id") != identity["attempt_id"]:
            raise PreconditionFailure(
                f"metrics panel {key}: attempt_id is not {identity['attempt_id']!r}"
            )
    papers = {r["paper_id"] for _, r in records if r.get("stage") == "panel"}
    if metrics.get("n_papers") != len(papers):
        raise PreconditionFailure(
            f"metrics n_papers {metrics.get('n_papers')} != {len(papers)} panel papers here"
        )
    ...
    if metrics.get("input_bindings") != bindings:
        raise PreconditionFailure(
            "metrics input bindings differ: synthesis, gold, decision overrides or severity input changed"
        )
    ...
    if unresolved or scored != expected:
        raise PreconditionFailure(
            "metrics decisions differ from the bound synthesis and decision overrides"
        )
...
def verified_class_a_audit(args, metrics: dict, work: Path) -> dict:
    ...
    if not {"expected_label", "venue_partition"}.issubset(audit.get("blinded_to") or []):
        raise PreconditionFailure(
            "class-A audit must attest blinding to expected_label and venue_partition"
        )
    ...
    syntheses = {
        scorer.panel_key(r): scorer.read_raw(work / "runs", r, "synthesis.md") for r in records
    }
    ...
    for key, entry in panels.items():
        ...
        if (
            entry.get("decision") not in scorer.DECISIONS
            or entry["decision"] != metrics["per_panel"][key]["decision"]
        ):
            raise PreconditionFailure(
                f"class-A audit {key}: decision differs from scored decision; resolve before building"
            )
        ...
        if key in overrides and excerpt != overrides[key]["raw"]:
            raise PreconditionFailure(
                f"class-A audit {key}: excerpt differs from decision override"
            )
...
def build_row(args) -> dict:
    ...
    if claim_errors:
        raise PreconditionFailure(
            "declared claims unsupported by the manifest: " + "; ".join(claim_errors)
        )
    judges = _load_json(args.judges, None)
    if not isinstance(judges, list) or not judges:
        raise PreconditionFailure(
            "--judges must be a non-empty JSON list of judge rows (Phase 3.5 output)"
        )
    ...
    blocked_runs = sorted(
        set(blocked) | set(metrics.get("blocked_runs", [])) | set(args.blocked_run)
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--work-dir", required=True, help="dispatcher work dir (records + manifest)"
    )
    ...
    parser.add_argument(
        "--decision-overrides", help="panel-keyed class-A overrides used by the scorer"
    )
    parser.add_argument(
        "--severity-classifications", help="exact severity input used by the scorer, when supplied"
    )
    parser.add_argument(
        "--class-a-audit",
        required=True,
        help="complete blind synthesis audit (calibration-class-a-audit/1)",
    )
    ...
    parser.add_argument(
        "--runs-ref",
        required=True,
        help="repo-relative dir the raw bundles + manifest are filed under",
    )
    parser.add_argument("--verdict", required=True)
    parser.add_argument("--caveat", action="append", default=[])
    parser.add_argument(
        "--claim", action="append", default=[], choices=("same_window", "ordering", "concurrency")
    )
    ...
    parser.add_argument(
        "--resolve-refs", action="store_true", help="also run checker R1-R5 (filed rows only)"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"measurement row: {args.out} (validated against heldout-measurement/1.1"
