    ...
    for i, rec in enumerate(art["items"]):
        ...
        if rec["priority"] == "must_fix":
            ...
            for field in ("partially_addressed", "made_worse_discriminator"):
                if field in op:
                    v.fail(
                        f"{path}.operationalization", f"P2 lighter form forbids {field!r} (§5.1)"
                    )
...
def validate_manifest(manifest: dict):
    ...
    if reports["present"]:
        if not patches["present"]:
            raise ManifestError(
                "manifest_incomplete",
                "manifest: apply_reports present with revision_patches absent (§11)",
            )
    ...
    for i, report in enumerate(report_payloads):
        ...
        if version >= (1, 2):
            ...
            if not isinstance(digest, str) or not _SHA256_RE.match(digest):
                raise ManifestError(
                    "manifest_incomplete",
                    f"apply report [{i}]: format >= 1.2 requires patch_digest",
                )
...
def check(
    ...
    p12_ids = {
        item["id"] for item in roadmap_items if item["priority"] in ("must_fix", "should_fix")
    }
    ...
    for item_id, rec in pre_by_item.items():
        ...
        if rec["priority"] != item["priority"]:
            fails.add(
                f"precommitment {item_id}: priority {rec['priority']} != roadmap {item['priority']}"
            )
    ...
    if (
        not manifest["artifacts"]["original_manuscript"]["present"]
        and verdict_record["escalation_exceptions"]
    ):
        fails.add(
    ...
    for rec in verdict_record["new_issues"]:
        ...
        if (
            not manifest["artifacts"]["original_manuscript"]["present"]
            and rec["attribution"] != "indeterminate"
        ):
            fails.add(
    ...
    for item_id, row in row_by_item.items():
        ...
        if ROW_PRIORITY_MAP[row["priority"]] != item["priority"]:
            fails.add(
                f"row {item_id}: priority {row['priority']} does not match the roadmap ({item['priority']})"
            )
    ...
    for rec in traceability["resolution_intents"]:
        ...
        if rec["answered_by"] == "system":
            ...
            if (
                intent_row is None
                or intent_row["priority"] != "MUST_FIX"
    ...
    if di["per_item"] != expected_per_item:
        fails.add(
            "decision_inputs.per_item does not equal the recomputed per-P1 operand list (roadmap must_fix order, §13)"
        )
    ...
    if di["residual_magnitude_counts"] != expected_magnitudes:
        fails.add(
            "decision_inputs.residual_magnitude_counts does not recompute from the PARTIALLY_ADDRESSED rows (§13)"
        )
    ...
    if di["p2_addressed_rate"] != {"numerator": expected_num, "denominator": len(p2_ids)}:
        fails.add(
            "decision_inputs.p2_addressed_rate does not recompute over FINAL P2 verdicts (§6)"
        )
    ...
    for item_id, row in row_by_item.items():
        if (
            ...
            and ROW_PRIORITY_MAP[row["priority"]] == "must_fix"
        ):
            if not _covering_resolution(item_id, row["final_verdict"]):
                pending.append(
                    f"G2(b): P1 diverges row {item_id} has no COVERING CrossModelResolution"
                )
    for rec in verdict_record["escalation_exceptions"]:
    ...
    if emitted_state == "aborted":
        ...
        if emitted_state == "user_review_required":
            ...
            for item_id in must_fix_order:
                ...
                p1_operands.append(
                    (row["final_verdict"], severity, _final_residual_magnitude(item_id))
                )
            p2_partial_magnitudes = [
                _final_residual_magnitude(item_id)
                for item_id in p2_ids
    ...
    try:
        ...
        for i, (path, entry) in enumerate(zip(args.apply_reports, report_entries)):
            ...
            try:
                ...
                raise ManifestError(
                    "manifest_incomplete", f"apply report [{i}]: invalid JSON ({exc})"
                )
