...
def check_links_resolve(root: Path) -> list[str]:
    ...
    for target in extract_link_targets(text):
        ...
        if failure is RelativeLinkFailure.ESCAPES_REPOSITORY:
            ...
            errors.append(
                f"CA-1: link target '{path_part}' escapes the repository (from {DOC_RELPATH})"
            )
        elif failure is RelativeLinkFailure.DOES_NOT_EXIST:
            errors.append(f"CA-1: link target '{path_part}' does not exist (from {DOC_RELPATH})")
        elif failure is RelativeLinkFailure.ANCHOR_ON_NON_MARKDOWN:
            errors.append(f"CA-1: anchor on non-markdown target '{target}'")
        elif failure is RelativeLinkFailure.ANCHOR_NOT_FOUND:
            if not path_part:
                errors.append(
                    f"CA-1: intra-doc anchor '{target}' has no matching heading in {DOC_RELPATH}"
                )
            else:
                errors.append(
...
def check_inbound_links(root: Path) -> list[str]:
    ...
    for rel in INBOUND_LINK_SURFACES:
        ...
        if not links_to(text, surface.parent, doc_abs):
            errors.append(
                f"CA-3: {rel} no longer links to {DOC_RELPATH.name} (#757 acceptance criterion)"
            )
    return errors
