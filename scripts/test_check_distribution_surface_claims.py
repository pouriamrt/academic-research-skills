...
def _write(
    root: Path, plugin=CLEAN_PLUGIN, marketplace=CLEAN_MARKETPLACE, plugin_raw: str | None = None
) -> Path:
    ...
    if marketplace is not None:
        (d / "marketplace.json").write_text(json.dumps(marketplace), encoding="utf-8")
    return root
...
def test_nonstandard_json_nan_fails_closed(tmp_path):
    ...
    _write(tmp_path, plugin=None, plugin_raw='{"description": "39 prompt roles", "score": NaN}')
    _fires(run(tmp_path), "D1")


def test_nonstandard_json_infinity_fails_closed(tmp_path):
    _write(tmp_path, plugin=None, plugin_raw='{"description": "39 prompt roles", "x": Infinity}')
    _fires(run(tmp_path), "D1")
...
def test_empty_plugins_list_fires_d2(tmp_path):
    _write(tmp_path, marketplace={"name": "f", "description": "ok", "plugins": []})
    _fires(run(tmp_path), "D2")


def test_production_grade_fires_d3(tmp_path):
    _write(tmp_path, plugin={"name": "f", "description": "Production-grade research pipeline."})
    _fires(run(tmp_path), "production-grade")


def test_production_grade_space_spelling_fires_d3(tmp_path):
    _write(tmp_path, plugin={"name": "f", "description": "A production grade pipeline."})
    _fires(run(tmp_path), "production-grade")


def test_agent_ensemble_fires_d3(tmp_path):
    _write(tmp_path, plugin={"name": "f", "description": "A 39-agent ensemble for research."})
...
def test_effectiveness_stems_fire_d3(tmp_path):
    for phrase in (
        ...
        "improves error rates",
        "outperforms manual review",
    ):
        errors = run(
            _write(tmp_path, plugin={"name": "f", "description": f"A pipeline with {phrase}."})
        )
        _fires(errors, "D3")
...
def test_marketplace_plugin_entry_description_covered(tmp_path):
    _write(
        ...
        marketplace={
            "name": "f",
            "description": "ok",
            "plugins": [{"name": "f", "description": "Production-grade skills."}],
        },
    )
...
def test_missing_count_token_fires_d4(tmp_path):
    _write(tmp_path, plugin={"name": "f", "description": "Contract-audited research pipeline."})
...
def test_unbindable_count_spelling_fires_d4(tmp_path):
    ...
    _write(tmp_path, plugin={"name": "f", "description": "A pipeline with 39 agents."})
    _fires(run(tmp_path), "D4")
...
def test_near_miss_spelling_agentic_fires_d4(tmp_path):
    ...
    _write(tmp_path, plugin={"name": "f", "description": "A 39-agentic workflow suite."})
    _fires(run(tmp_path), "D4")


def test_near_miss_spelling_singular_role_fires_d4(tmp_path):
    # "39 prompt role" (singular) is not the licensed plural spelling
    _write(tmp_path, plugin={"name": "f", "description": "A suite with 39 prompt role."})
    _fires(run(tmp_path), "D4")
