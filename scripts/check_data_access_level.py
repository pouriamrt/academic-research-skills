...
# - `data-analyst: raw`, `experiment-designer: raw`, `simulation-runner: raw`,
#   `lab-notebook: raw` — fork-derived under the same dirtiest-input rule. Each
#   reads user-supplied material with no gate ahead of it: data files
#   (data-analyst, simulation-runner `bootstrap`), the raw study description and
#   instrument text (experiment-designer), and free-form log entries
#   (lab-notebook, which verifies nothing despite its former `verified_only`
#   declaration).
EXPECTED_LEVELS = {
    "academic-paper": "raw",
    ...
    "deep-research": "raw",
    "data-analyst": "raw",
    "experiment-designer": "raw",
    "lab-notebook": "raw",
    "simulation-runner": "raw",
}
...
def run_all_checks(root: Path) -> list[str]:
    ...
    for skill_md in iter_skill_files(root):
        ...
        if not isinstance(metadata, dict):
            violations.append(
                f"{skill_md}: metadata must be a mapping/object, got {type(metadata).__name__}"
            )
            continue
    ...
    for name in sorted(set(EXPECTED_LEVELS) - seen):
        violations.append(f"EXPECTED_LEVELS pins '{name}' but no top-level {name}/SKILL.md exists")
    return violations
...
def main() -> int:
    ...
    if violations:
        ...
        return 1
    print("OK: all SKILL.md files declare a valid, pinned data_access_level.")
    return 0
