...
Fork adaptation: upstream also pins the zh-TW / zh-CN / ja-JP / ko-KR READMEs and
docs/PERFORMANCE.zh-TW.md. This fork deleted those in the v3.17.0 bilingual purge
(check_spec_consistency.check_bilingual_purge asserts they stay deleted), so their
pins are dropped here. The invariant itself is unchanged — every surviving surface
still has to carry the display label and the frozen internal tokens.
"""
...
REQUIRED = {
    ...
    "MODE_REGISTRY.md": ("Journal-Fit Reviewer quick assessment + key issues list",),
    "shared/mode_spectrum.md": ("Fixed Journal-Fit Reviewer quick-assessment format",),
    ...
    "academic-paper-reviewer/agents/field_analyst_agent.md": (
        # Fork: five-seat panel — the DA seat is load-bearing for the
        # DA-CRITICAL terminal gate in check_panel_synthesis.py.
...
FORBIDDEN_REVIEW_DISPATCH = {
    ...
    "docs/ARCHITECTURE.md": ("**Narrow re-review team**: eic_agent + editorial_synthesizer_agent",),
    "shared/model_tiering.md": (
...
FORBIDDEN_DISPLAY = {
    ...
    "academic-paper-reviewer/examples/subclaim_decomposition_example.md": ("EIC arbitration",),
}
