...
class WriteScopeGuardTest(unittest.TestCase):
    ...
    def test_in_scope_write_allowed(self):
        p = payload(
            "Write",
            {
                "file_path": os.path.join(self.ws, "phase2_investigation/annotated_bib.md"),
                "content": "x",
            },
            cwd=self.ws,
    ...
    def test_infra_manifest_denied_even_with_broad_glob(self):
        ...
        p = payload(
            "Write",
            {
                "file_path": os.path.join(self.ws, "scripts/ars_phase_scope_manifest.json"),
                "content": "x",
            },
            cwd=self.ws,
            agent_type="broad_test_agent",
        )
    ...
    def test_infra_hook_script_denied(self):
        ...
        p = payload(
            "Write",
            {
                "file_path": os.path.join(self.ws, "scripts/ars_write_scope_guard.py"),
                "content": "x",
            },
            cwd=self.ws,
            agent_type="broad_test_agent",
    ...
    def test_main_session_escape_allowed(self):
        ...
        self.assertTrue(
            d.get("absent_agent_type_advisory"),
    ...
    def test_non_bucket_a_agent_escape_allowed(self):
        ...
        p = payload(
            ...
            {"file_path": raw, "content": "x"},
            cwd=self.ws,
            agent_type="report_compiler_agent",
        )  # Bucket B, not in manifest
        d = self.decide(p)
        self.assertEqual(d["decision"], "allow")
...
class NormalizationRegressionTest(unittest.TestCase):
    ...
    def test_symlink_traversal_to_infra_denied(self):
        ...
        symlink_or_skip(
            self, os.path.join(self.ws, "phase2_investigation"), os.path.join(self.ws, "link")
        )
...
class BashBypassRegressionTest(unittest.TestCase):
    ...
    def _bash(self, cmd):
        return self.decide(
            payload("Bash", {"command": cmd}, cwd=self.ws, agent_type="bibliography_agent")
        )
    ...
    def test_all_historical_bash_bypasses_denied(self):
        ...
        for cmd in shapes:
            self.assertEqual(
                self._bash(cmd)["decision"], "deny", f"{cmd!r} must deny (all-Bash-deny)"
            )
    ...
    def test_deep_path_glob_match_does_not_crash(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.ws, deep_under), "content": "x"},
            cwd=self.ws,
            agent_type="bibliography_agent",
        )
        self.assertEqual(self.decide(p)["decision"], "allow")
...
class PluginRootInfraScopeTest(unittest.TestCase):
    ...
    def test_user_project_root_claude_md_allowed_main_session(self):
        ...
        p = payload(
            "Write", {"file_path": os.path.join(self.ws, "CLAUDE.md"), "content": "x"}, cwd=self.ws
    ...
    def test_user_project_dotclaude_claude_md_allowed_main_session(self):
        p = payload(
            "Write",
            {"file_path": os.path.join(self.ws, ".claude/CLAUDE.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "allow")
    ...
    def test_user_project_shared_agents_md_allowed(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.ws, "shared/agents/foo.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "allow")
    ...
    def test_user_project_hooks_sh_allowed(self):
        p = payload(
            "Write",
            {"file_path": os.path.join(self.ws, "hooks/install.sh"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "allow")
    ...
    def test_plugin_own_claude_md_allowed(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.plugin, "CLAUDE.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "allow")
    ...
    def test_plugin_own_dotclaude_claude_md_allowed(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.plugin, ".claude/CLAUDE.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "allow")
    ...
    def test_plugin_own_hooks_json_denied(self):
        p = payload(
            "Write",
            {"file_path": os.path.join(self.plugin, "hooks/hooks.json"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "deny")
    ...
    def test_plugin_own_shared_agents_md_denied(self):
        p = payload(
            "Write",
            {"file_path": os.path.join(self.plugin, "shared/agents/bar.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "deny")
    ...
    def test_plugin_own_toplevel_agents_md_denied(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.plugin, "agents/synthesis_agent.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "deny")
    ...
    def test_user_project_toplevel_agents_md_allowed(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.ws, "agents/my_agent.md"), "content": "x"},
            cwd=self.ws,
        )
        self.assertEqual(self.decide_in_project(p)["decision"], "allow")
    ...
    def test_bucket_a_cannot_reach_user_claude_md(self):
        ...
        p = payload(
            "Write",
            {"file_path": os.path.join(self.ws, "CLAUDE.md"), "content": "x"},
            cwd=self.ws,
            agent_type="bibliography_agent",
        )
    ...
    def test_dual_root_bucket_a_sibling_worktree_escape_still_allowed_for_main(self):
        ...
        try:
            p = payload(
                "Write",
                {"file_path": os.path.join(sibling, "notes.md"), "content": "x"},
                cwd=self.ws,
            )  # main session
            self.assertEqual(self.decide_in_project(p)["decision"], "allow")
        finally:
            ...
            inp = json.dumps(
                {
                    "tool_name": "Write",
                    "cwd": proj,
                    "tool_input": {"file_path": os.path.join(proj, "CLAUDE.md"), "content": "x"},
                }
            )
            out = subprocess.run(
            ...
            inp = json.dumps(
                {
                    ...
                    "cwd": proj,
                    "tool_input": {
                        "file_path": os.path.join(plug, "scripts/ars_write_scope_guard.py"),
                        "content": "x",
                    },
                }
            )
