...
APA_STAT_PATTERNS = {
    "t_test": re.compile(r"t\((\d+(?:\.\d+)?)\)\s*=\s*(-?\d+\.\d+),\s*p\s*=\s*([.<]?\s*\.?\d+)"),
    "f_test": re.compile(r"F\((\d+),\s*(\d+)\)\s*=\s*(-?\d+\.\d+),\s*p\s*=\s*([.<]?\s*\.?\d+)"),
    "chi_square": re.compile(r"chi-sq\((\d+)\)\s*=\s*(-?\d+\.\d+),\s*p\s*=\s*([.<]?\s*\.?\d+)"),
    "correlation": re.compile(r"r\((\d+)\)\s*=\s*(-?\.?\d+),\s*p\s*=\s*([.<]?\s*\.?\d+)"),
...
def parse_script_metadata(script_path: Path) -> ScriptMetadata:
    ...
    # Python version
    py_match = re.search(r"Python(?:\s+Version)?\s*:\s*(\d[\d.]+)", combined, re.IGNORECASE)
    if py_match:
    ...
    pkg_match = re.search(r"(?:Key\s+)?[Pp]ackages?\s*:\s*(.+)", combined, re.IGNORECASE)
    if pkg_match:
    ...
    if len(orig_rows) != len(replay_rows):
        divergences.append(
            FileDivergence(
                file_path=rel_path,
                divergence_type="numeric",
                details=(f"Row count mismatch: expected {len(orig_rows)}, got {len(replay_rows)}"),
...
def compare_apa_statistics(
    ...
    for stat_name, pattern in APA_STAT_PATTERNS.items():
        ...
        for idx, (orig_groups, replay_groups) in enumerate(zip(orig_matches, replay_matches)):
...
def generate_report(
    ...
    for r in results:
        ...
        if r.metadata:
            ...
            if r.metadata.execution_date:
                meta_items.append(f"Original Execution Date: {r.metadata.execution_date}")
        ...
        if r.status == "DIVERGED" and r.divergences:
            ...
            for div in r.divergences:
                lines.append(f"- **{div.divergence_type}** in `{div.file_path}`: {div.details}")
    ...
    lines.append("")
    lines.append(
        "*Report generated by `tools/replay_experiments.py` — academic-research-skills pipeline*"
    )
    lines.append("")
...
def run_replay(
    ...
    if dry_run:
        ...
        return [
            ScriptResult(script_name=m.script_name, status="SKIPPED", metadata=m) for m in scripts
        ]
