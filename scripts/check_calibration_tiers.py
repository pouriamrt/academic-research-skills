...
PROTOCOL_WITNESSES = {
    ...
    "directional score provenance": (
        ...
        "The Devil's Advocate's dedicated standard-mode format emits no 0-100 rubric score",
    ...
    "full-tier replicate disclosure": ("Runs per paper: <3|5> (ensembled)",),
...
REGISTRY_WITNESSES = ("Explicit 3-paper directional readout or default full Calibration Report",)

SPECTRUM_WITNESSES = ("Explicit 3×1 directional tier or default 5× ensemble (3× override)",)
...
DIRECTIONAL_REPORT_SECTION_WITNESSES = {
    ...
    "## Minor/Major boundary sub-matrix": ("<the Phase 2.5 four raw cells>",),
...
DIRECTIONAL_METRIC_NAME = re.compile(rf"\b{DIRECTIONAL_METRIC_ATOM}\b", re.IGNORECASE)
...
DIRECTIONAL_CLAUSE_BOUNDARY = re.compile(r"(?:[;!?]+|(?<!\d)\.|\.(?!\d))")
...
def _check_report_contract(
    ...
    for witness in preamble_witnesses:
        if witness not in preamble:
            errors.append(f"{PROTOCOL}: {label} contract preamble lost witness {witness!r}")
    ...
    for heading, witnesses in section_witnesses.items():
        ...
        if section is None:
            errors.append(f"{PROTOCOL}: {label} contract section {heading!r} is unreadable")
            continue
        for witness in witnesses:
            if witness not in section:
                errors.append(
                    f"{PROTOCOL}: {label} contract section {heading!r} lost witness {witness!r}"
                )
...
def check(root: Path) -> list[str]:
    ...
    for label, block in (
        ...
        if _reports_lu_numeric_comparison(block):
            errors.append(f"{PROTOCOL}: {label} reports forbidden Lu numeric comparison")
    ...
    if full_report is not None:
        for witness in (
            ...
            if witness not in full_report:
                errors.append(f"{PROTOCOL}: full report honesty lost coverage witness {witness!r}")
