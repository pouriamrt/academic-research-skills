...
def _init(
    tmp_path: Path, resolved_inputs, *, target: str = "review-001", prior: Path | None = None
) -> Path:
...
def _option(*, new_data: bool = False, intent: str = "no") -> dict:
    return {
        ...
        "resource_effort": {
            ...
            "description": "Collect an additional sample."
            if new_data
            else "Revise one claim sentence.",
        },
...
def _findings(manifest: dict, *, no_remedy: bool = False) -> dict:
    ...
    row = {
        ...
        if no_remedy
        else _option(new_data=True, intent="yes_author_choice_required"),
        "stronger_option_reason": "No stronger option is honest without changing the study."
        if no_remedy
        else None,
        "no_honest_remedy_reason": "The missing observations cannot be reconstructed."
        if no_remedy
        else None,
    }
    return {
...
def _validate_findings(tmp_path: Path, resolved_inputs, manifest_path: Path, findings: dict) -> int:
    context_path, registry_path, _, _ = resolved_inputs
...
def test_schema_accepts_positive_manifest_and_findings(tmp_path, resolved_inputs) -> None:
    ...
    Draft202012Validator(
        load_json(CONTRACTS / "constructive_review_findings.schema.json")
    ).validate(_findings(manifest))
...
def test_schema_rejects_duplicate_consumer_and_wrong_role(tmp_path, resolved_inputs) -> None:
...
def test_findings_schema_rejects_new_data_flag_mismatch(tmp_path, resolved_inputs) -> None:
    manifest = _manifest(_init(tmp_path, resolved_inputs))
...
def test_init_and_validate_round_trip(tmp_path, resolved_inputs) -> None:
    ...
    assert [
        p["criterion_id"] for p in manifest["context_authority"]["selected_criteria"]
    ] == context["selected_criterion_ids"]
    assert (
        binding.main(
            [
                "validate",
...
def test_complete_requires_three_consumers_and_five_external_seats(
...
def test_predecessor_context_cannot_silently_bind_to_live_registry(
    ...
    predecessor["criteria"] = [
        item for item in predecessor["criteria"] if item["criterion_id"] not in PROVING_SET_IDS
    ]
    ...
    assert (
        binding.main(
            [
                "init",
                "--context",
                str(linked_parent / "context.json"),
                "--context-ref",
                "phase0/context.json",
                "--registry",
                str(registry_path),
...
def test_artifact_with_second_binding_marker_is_rejected(tmp_path, resolved_inputs) -> None:
    manifest_path = _init(tmp_path, resolved_inputs)
    manifest = _manifest(manifest_path)
...
def test_missing_or_changed_parallel_conflict_line_is_rejected(tmp_path, resolved_inputs) -> None:
    manifest_path = _init(tmp_path, resolved_inputs)
    manifest = _manifest(manifest_path)
...
def test_nonblocking_only_finding_fails(tmp_path, resolved_inputs) -> None:
    ...
    pointer = next(
        item
        for item in manifest["context_authority"]["selected_criteria"]
