...
from _calibration_pdf_text import (
    TEXT_NORMALIZATION,
    pdf_facts,
    pypdf,
    sha256_hex,
)
from _e4_evidence import EvidencePathError, assert_plain_file
from dispatch_e4_panel import (
    AGENT_DIR,
    AGENT_FILES,
    DATA_BOUNDARY,
    Bundle,
...
def credential_preflight(
    environ=None, *, opener=_PREFLIGHT_OPENER.open, timeout: float = 5.0
) -> str:
        ...
        try:
            ...
            if _is_auth_failure(failure):
                ...
                raise CredentialRejected(
                    call.label,
                    f"[TRANSPORT: credential rejected — not retried; evidence {location}]",
...
def load_frozen_card(work: Path, paper: str, seat: str) -> str:
    ...
    if not path.is_file():
        raise PreconditionFailure(f"no frozen Card #{index} for {paper}; run the cards stage first")
...
def _load_attempt_records(work: Path) -> tuple[list[tuple[str, dict]], list[tuple[str, dict]]]:
    ...
    for frozen in (
        sorted((work / "cards").glob("*/frozen.json")) if (work / "cards").is_dir() else []
    ):
    ...
    calls = [
        {
            ...
            **{k: v for k, v in row.items() if k != "call_id"},
        }
...
def _validate_manifest(manifest: dict) -> None:
    ...
    stamps += [
        (f"execution_window.{k}", window[k]) for k in ("started_at", "completed_at") if k in window
    ]
