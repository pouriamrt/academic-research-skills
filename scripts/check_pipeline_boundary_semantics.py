...
# ---------------------------------------------------------------------------
CONTENT_LOCKS = {
    "academic-pipeline/SKILL.md": "e18f681ced6c51f732b9e87320f7865f9f23476371b3f5d429bdde5f6b5b76fc",
...
INV1_FRAGMENTS = {
    ...
    SM: "| checkpoint (post-1) | Stage 2 | User confirms + no experiment routing flags | handoff RQ Brief + Methodology Blueprint + Bibliography + Synthesis |",
    ORCH: "| Stage 1 -> 2 (no experiment) | RQ Brief, Annotated Bibliography, Synthesis Report, Methodology Blueprint, Concept Lineage Report, INSIGHT Collection |",
}
...
INV2_SKILL_ROUTING = "6. **Stage 3' RE-REVIEW** -> Accept|Minor -> Stage 4.5 / Major -> **Experiment Re-Entry Check** -> Stage 4' (last experiment opportunity)"
...
INV2_ORCH_AFFIRMATIVE = {
    ...
    "coaching-handoff-if-major": "| Stage 3' -> **experiment check** -> **coaching** -> 4' | New Revision Roadmap (if Major) |",
}
...
S6_AUTHORITY_LITERALS = {
    ...
    "change-requests-not-ack": "Change requests (content corrections, format changes) keep Stage 6 `in_progress` — they are not acknowledgements",
    "ack-outcome": "On acknowledgement: state_tracker marks Stage 6 `completed` and sets the pipeline global state to `completed`",
    "post-delivery-prompt": "After delivering the Process Record (MD + PDF, English-only), the orchestrator prompts for a terminal acknowledgement",
...
TRACKER_PREREQ_STAGE2_ROW = "| Stage 2 | None (but Stage 1 output recommended) | RQ Brief, Methodology Blueprint, Bibliography, Synthesis; Experiment Design + Results + Lab Record (if Stage 1.5 executed) |"
