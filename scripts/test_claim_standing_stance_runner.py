...
def test_happy_path_produces_valid_record_and_evidence_rows():
    ...
    row_schema = json.loads((CONTRACTS / "evidence/evidence_row_v1_3.schema.json").read_bytes())
    ...
    assert record["identity"]["candidate_ledger_sha256"] == ledger_value["candidate_ledger_sha256"]
    assert record["identity"]["stance_plan_sha256"] == ledger.digest(plan["stance_plan"])
    ...
    assert (
        abstract.encode("utf-8")[span["start"] : span["end"]].decode("utf-8")
...
def test_consent_gate_refuses_plans_without_stance_authorization():
    ...
    with pytest.raises(runner.StanceError, match="retrieval_plus_stance"):
        runner.run_stance(plan_retrieval_only, ledger_value, transport=FakeTransport())
...
def test_every_transport_call_records_a_transmission_event():
    ...
    assert event["content_classes"] == ["claim_and_selected_evidence_to_stance_provider"]
...
def test_forged_cross_candidate_evidence_is_rejected():
    plan, ledger_value = _stance_setup()
    record, evidence_rows, _ = runner.run_stance(plan, ledger_value, transport=FakeTransport())
    ...
    resealed["rows"][0]["evidence_row_refs"][0]["row_sha256"] = forged[0]["row_sha256"]
    resealed["stance_record_sha256"] = ledger.bound_digest(resealed, "stance_record_sha256")
...
def test_fabricated_excerpt_fails_span_replay():
    ...
    forged[0]["excerpt"]["excerpt_sha256"] = ledger.text_digest("totally fabricated excerpt")
    forged[0]["row_sha256"] = ledger.bound_digest(forged[0], "row_sha256")
    resealed = copy.deepcopy(record)
    resealed["rows"][0]["evidence_row_refs"][0]["row_sha256"] = forged[0]["row_sha256"]
    resealed["stance_record_sha256"] = ledger.bound_digest(resealed, "stance_record_sha256")
...
def test_forged_stance_value_fails_raw_output_replay():
    ...
    forged["stance_record_sha256"] = ledger.bound_digest(forged, "stance_record_sha256")
    with pytest.raises(runner.StanceError, match="replay"):
...
def test_blank_lines_in_judge_output_are_parse_errors():
    ...
    record, _, _ = runner.run_stance(plan, ledger_value, transport=FakeTransport(output))
...
def test_semantic_validator_fails_closed_on_tampering():
    ...
    bad_sum["stance_record_sha256"] = ledger.bound_digest(bad_sum, "stance_record_sha256")
    ...
    missing_row["stance_record_sha256"] = ledger.bound_digest(missing_row, "stance_record_sha256")
    ...
    stale["stance_record_sha256"] = ledger.bound_digest(stale, "stance_record_sha256")
...
def test_record_hash_binds_the_whole_record():
    plan, ledger_value = _stance_setup()
    record, evidence_rows, _ = runner.run_stance(plan, ledger_value, transport=FakeTransport())
    assert record["stance_record_sha256"] == ledger.bound_digest(record, "stance_record_sha256")
