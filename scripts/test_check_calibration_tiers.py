...
def test_calibration_restart_cannot_resume_at_the_failed_panel(tree: Path) -> None:
    rel = CROSS_MODEL
    expected = "new `attempt_id`, an empty aggregate, and a restart at paper 1 / replicate 1"
    text = (tree / rel).read_text(encoding="utf-8")
    assert expected in text
...
def test_directional_generic_error_profile_claim_is_rejected(tree: Path, claim: str) -> None:
    _mutate(
        tree,
        PROTOCOL,
        "## Directional raw counts\n- lenient: <n>",
...
def test_full_report_never_globalizes_partial_dimension_gold(tree: Path, tmp_path: Path) -> None:
