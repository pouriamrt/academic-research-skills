...
from _skill_lint import norm_ws as _norm
...
COVERAGE_RECEIPT_SKELETON = "### Coverage Receipt (only when Strengths or Weaknesses is empty)"
...
QUOTA_PATTERNS = (
    ...
    re.compile(
        ...
        rf"\s+to\s+{_QUOTA_NUMS}\s+{_QUOTA_GAP}{_QUOTA_NOUNS}\b",
        re.I,
    ),
    re.compile(
        rf"\bat (?:least|minimum|most)\s+{_QUOTA_NUMS}\s+{_QUOTA_GAP}{_QUOTA_NOUNS}\b", re.I
    ),
    ...
    re.compile(
        rf"\b(?:list|find|include|provide|identify|give|report|produce|select|choose|pick|name)\s+"
        rf"(?:exactly\s+)?{_QUOTA_NUMS}\s+{_QUOTA_GAP}{_QUOTA_NOUNS}\b",
        re.I,
    ),
    re.compile(
        rf"\b(?:no more than|a maximum(?: of)?|maximum of|up to)\s+{_QUOTA_NUMS}\s+{_QUOTA_GAP}{_QUOTA_NOUNS}\b",
        re.I,
    ),
    ...
    re.compile(
        rf"\bbetween\s+{_QUOTA_NUMS}\s+and\s+{_QUOTA_NUMS}\s+{_QUOTA_GAP}{_QUOTA_NOUNS}\b", re.I
    ),
...
def check(root: Path) -> list[str]:
    ...
    for rel in SCORING_AGENTS:
        ...
        elif _norm(FINDING_CONTRACT_HEADING) not in _norm(phase2) or _norm(
            FINDING_CONTRACT_BODY
        ) not in _norm(phase2):
            errors.append(
        ...
        if phase2 is not None and _norm(BAND_ANCHORS) not in _norm(phase2):
            errors.append(
                f"{rel}: B1 band anchors missing from the delivered Phase 2 Finding Contract"
            )
    ...
    if da_output is None:
        ...
        da_output = ""
    required_cols = {
        "CRITICAL": ("Evidence Anchor", "Confidence"),
    ...
    if da_phase2 is not None and _norm(BAND_ANCHORS) not in _norm(da_phase2):
        errors.append(
            f"{DA_REL}: B1 band anchors missing from the delivered Phase 2 Finding Contract"
        )
    ...
    _require(
        ...
        tmpl_norm,
        TEMPLATE_REL,
        "**Critical/Major findings MUST carry an adequate anchor of an applicable type** (#574 A2)",
        "MUST-anchor rule",
    )
    _require(
        errors,
    ...
    _require(
        errors, tmpl_norm, TEMPLATE_REL, COVERAGE_RECEIPT_COVERS, "receipt polarity (Covers) line"
    )
    _require(
        errors,
        tmpl_norm,
    ...
    _require(
        ...
        tmpl_norm,
        TEMPLATE_REL,
        "Tone is register, not severity (#574 B1)",
    ...
    for pat in QUOTA_PATTERNS:
        if pat.search(tmpl):
            errors.append(
                f"{TEMPLATE_REL}: finding-quota regression: pattern {pat.pattern!r} present"
            )
    ...
    _require(
        ...
        "Schema 6 canonical-severity sentence",
    )
    ...
    for row in (
        "| `evidence_anchor` | object | *(optional, #574 A2)*",
    ...
    _require(
        ...
        schemas_norm,
        SCHEMAS_REL,
        'REQUIRED when `anchor_type = "text"`',
        "Schema 6 conditional quote member",
    )
    _require(
        errors,
        schemas_norm,
        SCHEMAS_REL,
        'REQUIRED when `anchor_type = "absence"`',
        "Schema 6 conditional absence members",
    )
    ...
    _require(
        ...
        synth_norm,
        SYNTH_REL,
        "TRANSPORTED, never re-derived (#574 A3)",
        "severity-transport rule",
    )
    _require(
        errors, synth_norm, SYNTH_REL, "[SEVERITY-SOURCE: letter-fallback]", "severity fallback tag"
    )
    _require(
        errors,
        synth_norm,
    ...
    _require(
        ...
        synth_norm,
        SYNTH_REL,
        "never a base rate or target distribution (#574 B1)",
        "B1 accept rule",
    )
    ...
    _require(
        ...
        synth_norm,
        SYNTH_REL,
        "**Unresolved-dissent principle**",
        "symmetric unresolved-dissent arbitration rule (B1)",
    )
    if "rather than directly dismissing" in synth:
        errors.append(
            f"{SYNTH_REL}: asymmetric arbitration regression ('lean toward … rather than directly dismissing')"
        )
    _require(
        errors,
        synth_norm,
    ...
    if "lean toward Major Revision" in synth:
        errors.append(
            f"{SYNTH_REL}: directional split prior regression ('lean toward Major Revision')"
        )
    ...
    _require(
        ...
        "sub-claim severity inheritance (A3)",
    )
    ...
    _require(
        ...
        "unresolved-dissent principle (#574 B1)",
        "arbitration rationale vocabulary (B1)",
    )
    if "conservative principle" in dt:
        errors.append(
            f"{DECISION_TEMPLATE_REL}: asymmetric arbitration regression ('conservative principle')"
        )
    ...
    if "uncommon in practice" in std:
        errors.append(
            f"{STANDARDS_REL}: qualitative base-rate cue regression ('uncommon in practice')"
        )
    _require(
        errors,
        std_norm,
    ...
    _require(
        ...
        std_norm,
        STANDARDS_REL,
        "this table assigns NO severities (#574 A3)",
        "no-severity-assignment note (A3)",
    )
    if "| Situation | Severity" in std:
        errors.append(
            f"{STANDARDS_REL}: severity-assignment column regression in the cross-dimension table"
        )
    _require(
        errors,
        std_norm,
    ...
    if "Lean toward conservative strategy" in std:
        errors.append(
            f"{STANDARDS_REL}: asymmetric split-handling regression ('Lean toward conservative strategy')"
        )
    _require(
        errors,
        std_norm,
        STANDARDS_REL,
        "Affirm genuine merits where they exist",
    ...
    _require(
        ...
        "EVERY additional weakness in the report",
        "unbounded response skeleton (A1)",
    )
    ...
    for mirror_heading, next_heading in (
        ("## Response to Reviewer 2 (Domain Expert)", "## Response to Reviewer 3"),
        (
    ...
    _require(
        ...
        stats_norm,
        STATS_REL,
        "never copied from the triage label",
        "triage-to-enum rule (A3)",
    )
    ...
    _require(
        ...
        _norm(fa),
        FIELD_ANALYST_REL,
        "tone changes wording, never the verdict",
        "developmental-register rule (B1)",
    )
    ...
    if schema7 is None:
        ...
        for row in (
            "| `severity` | enum | *(optional, #574 A3)* Transported Schema 6 "
            ...
            _require(
                errors, s7_norm, SCHEMAS_REL, row, "Schema 7 RoadmapItem transported-field row"
            )
        _require(
            errors,
            s7_norm,
    ...
    _require(
        ...
        synth_norm,
        SYNTH_REL,
        "never an automatic veto (#574 B1)",
        "DA-CRITICAL adjudication rule (B1)",
    )
    ...
    if (
        "Decision cannot be Accept (Checkpoint Rule #4)" in skill
    ...
    _require(
        ...
        dt_norm,
        DECISION_TEMPLATE_REL,
        "transported from the reviewer cards (#574 A3)",
        "roadmap severity-transport note",
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--root", type=Path, default=REPO_ROOT, help="tree root to lint (default: repo root)"
    )
    ...
    if errors:
        ...
        return 1
    print(
        "check_reviewer_finding_contract: OK (A1 quotas/receipt + A2 anchors + "
