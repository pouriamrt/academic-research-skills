...
def run_cards(env, tmp_path, analysis=ANALYSIS):
    return mod.main(base_argv(env, "cards") + scripted(tmp_path, {"field_analyst": [analysis]}))
...
def test_cards_stage_freezes_four_cards(env, tmp_path):
    ...
    for index, expected in (
        ...
        (3, "domain config"),
        (4, "perspective config"),
    ):
        text = (cards_dir / f"card{index}.md").read_text(encoding="utf-8")
        assert expected in text
        assert "panel-wide notes" not in text
    frozen = json.loads((cards_dir / "frozen.json").read_text(encoding="utf-8"))
...
def test_cards_stage_refuses_missing_card(env, tmp_path):
    ...
    blocked = json.loads(
        (env["work"] / "runs" / "blocked-cards-p1.json").read_text(encoding="utf-8")
    )
...
def test_panel_complete_record_and_raw(env, tmp_path):
    ...
    record = json.loads(
        (env["work"] / "runs" / "2026-08-07-p1-r1.json").read_text(encoding="utf-8")
    )
    ...
    assert (raw / "synthesis.md").read_text(encoding="utf-8") == SYNTHESIS
...
def test_panel_without_frozen_cards_aborts(env, tmp_path):
    ...
    assert rc == 1
    blocked = json.loads(
        (env["work"] / "runs" / "blocked-2026-08-07-p1-r1.json").read_text(encoding="utf-8")
    )
...
def test_symlink_in_pdf_cache_refused(env, tmp_path):
    os.symlink(env["corpus"] / "manifests" / "gold_labels.json", env["cache"] / "labels.json")
...
def test_records_carry_per_call_timing_and_hashes(env, tmp_path):
    ...
    frozen = json.loads((env["work"] / "cards" / "p1" / "frozen.json").read_text(encoding="utf-8"))
...
class _RaisingTransport:
    ...
    def __init__(
        self, failures: dict[str, list[TransportFailure]], responses: dict[str, list[str]]
    ):
...
def test_credential_preflight_is_inconclusive_on_network_trouble_and_skips_without_key():
    ...
    assert mod.credential_preflight({"ANTHROPIC_API_KEY": "k"}, opener=opener).startswith(
        "inconclusive"
    )
    ...
    assert (
        mod.credential_preflight({"ANTHROPIC_API_KEY": "k"}, opener=opener_500)
    ...
    assert mod.credential_preflight({"ANTHROPIC_API_KEY": "   "}, opener=never).startswith(
        "skipped"
    )
...
def test_preflight_gate_preserves_success_and_explicit_default_fallback(
    env, monkeypatch, required, outcome
):
    ...
    record = json.loads((env["work"] / "cards/p1/frozen.json").read_text(encoding="utf-8"))
...
def test_manifest_stage_keeps_retry_attempt_numbers_and_skips_failed_rows(env, tmp_path):
    ...
    assert mod.main(base_argv(env, "panel") + scripted(tmp_path, panel_responses())) == 0
    assert mod.main(_manifest_argv(env)) == 0
    calls = json.loads((env["work"] / "execution-manifest.json").read_text(encoding="utf-8"))[
        "calls"
    ]
    assert len(calls) == 7
...
def test_credential_preflight_never_follows_redirects_and_skips_plain_http():
    ...
    def opener(request, timeout):
        raise urllib.error.HTTPError(
            request.full_url, 302, "Found", {"Location": "https://elsewhere.example/"}, None
        )

    assert (
        mod.credential_preflight({"ANTHROPIC_API_KEY": "k"}, opener=opener)
        == "inconclusive: HTTP 302"
    )

    def never(request, timeout):  # pragma: no cover
        raise AssertionError("plain http must not carry the key")

    outcome = mod.credential_preflight(
        {"ANTHROPIC_API_KEY": "k", "ANTHROPIC_BASE_URL": "http://proxy.local"}, opener=never
    )
...
def test_manifest_refuses_edited_raw_output_and_foreign_stage(env, tmp_path):
    ...
    synthesis.write_text(SYNTHESIS.replace("Major Revision", "Accept"), encoding="utf-8")
...
def test_structured_auth_failure_is_not_retried(env, tmp_path):
    structured = TransportFailure(
        "field_analyst",
        "[TRANSPORT: result error_during_execution] Failed to authenticate.",
        stdout="Failed to authenticate. API Error: 401 API key is invalid.",
    ...
    assert (
        (raw / "field_analyst.attempt1.transport-stream.jsonl")
...
def test_cli_structured_auth_diagnostic_stops_after_one_call(
    env, tmp_path, monkeypatch, exit_code, partial
):
    events = [
...
def test_interrupt_preserves_a_blocked_stage_and_call_ledger(env, tmp_path, stage, label):
    ...
    record = json.loads((env["work"] / "runs" / name).read_text(encoding="utf-8"))
...
def test_successful_calls_keep_the_raw_stream_when_the_transport_offers_it(env, tmp_path):
    ...
    raw = env["work"] / "cards" / "p1" / "raw"
    assert (
        (raw / "field_analyst.transport-stream.jsonl")
        .read_text(encoding="utf-8")
        .startswith('{"type":"assistant"}')
    )
    assert (raw / "field_analyst.md").read_text(encoding="utf-8") == ANALYSIS
