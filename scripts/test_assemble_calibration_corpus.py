...
def test_pool_ids_written_and_hashed(tmp_path):
    ...
    ids = (tmp_path / "corpus" / "pool_accepted_ids.txt").read_text(encoding="utf-8").split()
            ...
            meta["papers"].append(
                {
                    "paper_id": pid,
                    "title": f"Title {pid}",
                    "venue_string": "ICLR 2026 Poster"
                    if cls == "accepted"
                    else "Submitted to ICLR 2026",
...
def test_freeze_and_verify_roundtrip(tmp_path):
    ...
    run_freeze(env)
    papers = json.loads((tmp_path / "corpus" / "papers.json").read_text(encoding="utf-8"))
    labels = json.loads((tmp_path / "manifests" / "gold_labels.json").read_text(encoding="utf-8"))
...
def test_papers_json_carries_no_decision_vocabulary(tmp_path):
    env = freeze_env(tmp_path)
    run_freeze(env)
    payload = json.loads((tmp_path / "corpus" / "papers.json").read_text(encoding="utf-8"))
...
def test_freeze_page_cap_promotes_next(tmp_path):
    env = freeze_env(tmp_path)
    selection = json.loads(env["selection_path"].read_text(encoding="utf-8"))
    ...
    meta = json.loads(env["meta_path"].read_text(encoding="utf-8"))
    meta["papers"].append(
        {
            "paper_id": spare,
            "title": f"Title {spare}",
            "venue_string": "ICLR 2026 Poster",
            "decision_note_id": f"dec-{spare}",
...
def test_verify_detects_pdf_tamper(tmp_path):
    env = freeze_env(tmp_path)
    run_freeze(env)
    victim = json.loads((tmp_path / "corpus" / "papers.json").read_text(encoding="utf-8"))[
        "papers"
    ][0]
...
def test_freeze_refuses_when_layout_separates_the_classes(tmp_path, monkeypatch):
    ...
    selection = json.loads(env["selection_path"].read_text(encoding="utf-8"))
    monkeypatch.setattr(
        mod, "first_page_text", _first_page_by_class(selection, CAMERA_READY, SUBMISSION)
    )
...
def test_freeze_records_uniform_layout_and_verify_recomputes_it(tmp_path, monkeypatch):
    ...
    selection = json.loads(env["selection_path"].read_text(encoding="utf-8"))
    monkeypatch.setattr(
        mod, "first_page_text", _first_page_by_class(selection, SUBMISSION, SUBMISSION)
    )
    run_freeze(env)
    payload = json.loads((tmp_path / "corpus" / "papers.json").read_text(encoding="utf-8"))
