...
def load_rows(model: str) -> list[dict]:
    if RESULTS_DIR is not None:
        return [
            ...
            for p in sorted((RESULTS_DIR / model).glob("*.json"))
        ]
    return [
        json.loads(line)
        for line in (HERE / f"run7_receipts_{model}.jsonl").read_text(encoding="utf-8").splitlines()
        if line
    ]
...
for model in MODELS:
    ...
    for row in rows:
        ...
        if (
            ...
            and row.get("reasoning_effort") != "provider-default (env unset)"
        ):
            raise SystemExit(
    ...
    misfires = [
        {
            "ref": r["ref_id"],
            "repeat": r["repeat"],
            "reason": (r["receipt"] or {}).get("reason_code") or r["error"],
        }
        for r in rows
        if (r["receipt"] and r["receipt"].get("reason_code") in SHAPE_GUARD_CODES)
        or (r["receipt"] is None)
    ]
    ...
    false_dis = sum(
        1 for v in real_verdicts.values() if v in DISAGREE or v == "INDETERMINATE"
    ) / len(real_ids)
...
if len(fleet_days) != 1:
    raise SystemExit(
        f"MIXED-DATE FLEET: calls span {sorted(fleet_days)} — the gate requires one same-day paired run."
    )

b, c = summary["gpt-5.5"], summary["gpt-5.6-sol"]
gate = {
    "1_grounded_completion (cand >= base - 5pp)": c["m1_grounded_completion"]
    >= b["m1_grounded_completion"] - 0.05,
    "2_recall (cand >= base - 5pp AND >= 0.80)": c["m2_fabrication_recall"]
    >= b["m2_fabrication_recall"] - 0.05
    and c["m2_fabrication_recall"] >= 0.80,
    "3_false_disagreement (cand <= base + 5pp)": c["m3_false_disagreement"]
    <= b["m3_false_disagreement"] + 0.05,
    ...
    "4_zero_shape_guard_misfires (both fleets)": len(c["m4_shape_guard_misfires"]) == 0
    and len(b["m4_shape_guard_misfires"]) == 0,
    "5_p95_latency (cand <= 2x base)": c["m5_p95_latency_s"] <= 2 * b["m5_p95_latency_s"],
}
all_pass = all(gate.values())
print(
    json.dumps(
        {"models": summary, "gate": gate, "all_pass": all_pass}, ensure_ascii=False, indent=1
    )
)
if not all_pass:
