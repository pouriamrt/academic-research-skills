...
BUCKET_BCD_AGENT_FILES = [
    "deep-research/agents/concept_lineage_agent.md",
...
def run_checks() -> list[str]:
    ...
    # I1 — roster size
    if len(BUCKET_A_AGENT_FILES) != 22:
        errors.append(
            f"I1: BUCKET_A_AGENT_FILES has {len(BUCKET_A_AGENT_FILES)} entries, expected 22 "
            "(classification doc: A=22; fork v3.17.0 dropped abstract_bilingual_agent). Update in lockstep with check_v3_9_2_phase_boundary.py."
        )
    if len(BUCKET_BCD_AGENT_FILES) != 17:
        errors.append(
            f"I1: BUCKET_BCD_AGENT_FILES has {len(BUCKET_BCD_AGENT_FILES)} entries, expected 17 (fork v3.19.0 added concept_lineage_agent)."
        )
