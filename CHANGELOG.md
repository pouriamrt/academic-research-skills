...
> **Version-number collision note:** the upstream release entries `[3.14.0]`–`[3.17.0]` dated 2026-07 below describe **upstream's own release line**, which reuses version numbers this fork had already assigned to its own 2026-05 releases. The fork merged their content here, in `[3.20.0]`.

**Adopted from upstream (highlights):**
- **#528/#529/#535 — Stage 5/6 boundary semantics.** Stage 5 entry gate = the single MANDATORY finalization checkpoint; Stage 6 gains a decline path + terminal-acknowledgement vocabulary (`finish`/`end`/`done`/`confirm`); five pipeline surfaces carry whole-file sha256 content locks (`scripts/check_pipeline_boundary_semantics.py`). Fork adaptation: terminal acknowledgement is interactive-mode-only — AUTO mode (fork default) proceeds as if acknowledged on delivery; lint pins re-anchored to the fork's experiment-stage tables and diagram, and the locks re-pinned to the merged fork surfaces.
- **#527/#536 — Canonical `[CROSS-MODEL-HANDOFF v1]` envelope** + dispatcher consumer contract (`scripts/cross_model_handoff.py` normative grammar; `scripts/check_cross_model_handoff_contract.py`); #523 blind-checkpoint transport moved to the dispatching layer. Cross-model blind disagreement checkpoints added to `research_architect_agent` (design freeze) and `editorial_synthesizer_agent` (editorial decision) — union-merged alongside the fork's Experiment Pipeline Routing Gate and Machine-Readable Verdict Block.
- **#517/#520 — Model tiering** (`ARS_MODEL_TIERING=economy|quality-boost`, default unset = byte-equivalent). Fork extension: the frozen classification grows 39 → **61 agents** (29 judgment / 32 execution) — the 4 experiment skills' 22 agents + `concept_lineage_agent` classified; fork-deleted `abstract_bilingual_agent` dropped from the manifest.
- **#518/#519 — Cross-model gate hardening**: risk-stratified sampling (HIGH-IMPACT references 100% at both gates), id-status allowlist (`CROSS_MODEL_ID_STATUS`), promotion bakeoff; the once-planned generic 6th reviewer is retired, not deferred. #515: GPT-5.6 Sol provisional verifier + `ARS_CROSS_MODEL_REASONING_EFFORT`.
- **#510/#532 — Executable sprint-contract panel checker** (`scripts/check_panel_synthesis.py`) + majority-formula fix (simple majority `⌊N/2⌋+1`).
- **#511/#533/#534 — Machine-readable degradation registry** (`shared/contracts/degradation_registry.json` + lint) and hermetic transport-fixture integration test for the citation gate.
- **#514/#521/#524 — Tools allowlist** for the three plugin agents (`tools: Read, Write, Edit, Grep, Glob`) + Unicode-normalizing defrift lock (`scripts/check_tools_allowlist.py`).
- **#500/#502 — CARS intro-rhetoric + title-crafting reference** (`academic-paper/references/intro_title_rhetoric_guide.md`) wired into `draft_writer_agent`.
- **#501/#503/#504/#505/#507 — WP advisory generalization** (noun-swap test operative, off-list shells can fire) + sharpened exemption clause; 48-item held-out acceptance set under `evals/heldout/rq_framing_offlist/`.
- **#495/#496 — OpenAlex API-key auth, budget-aware 429 handling, arXiv ToU-aligned backoff.** #484/#506: reviewer calibration protocol documents LLM-as-judge leniency direction (FARS anchor).
- **#480 — Claude Science importability**: `marketplace.json` declares explicit skill paths — fork declares all **8** skills (upstream declares 4). #479: eval-harness PR comments render as verdict + table. #473: platform-port reminder CI. #483/#487/#486: CHANGELOG-covers-merges pre-tag gate, version-consistency invariants 9–11 + tag-version-match gate, command-invariants gate (fork announce inventory: 16 commands). #497/#498: `THIRD_PARTY.md`.
- **#476–#478, #489/#490 — prompt-debt retirement rounds 1+2** (stale enforcement sentences fixed repo-wide; socratic auto-end contradiction resolved; scaffolds trimmed); audit reports under `audits/`.
- **Vendored release-discipline toolkit** under `tools/release-discipline/` + `.command-invariants.toml` manifest.

**Fork-side conflict resolution (89 conflicted files):**
- Fork-wins: English-only surfaces (descriptions, triggers, README set), fork version line, experiment-pipeline tables/diagrams/enums, auto-by-default checkpoint flow, Schema 19/20 renumbering, `check_spec_consistency.py` (no bilingual checkers; upstream's ko/ja/zh checker tests stripped from `scripts/test_check_spec_consistency.py`).
- Union: `research_architect_agent` (fork routing gate + upstream design-freeze checkpoint), `editorial_synthesizer_agent` (fork verdict block + upstream decision checkpoint), `academic-paper/SKILL.md` (fork examples + upstream references list, 27 files), pipeline SKILL Stage 4.5 steps + v3.9.2 enforcement note.
- Adopted upstream fixes: Stage 3' Minor no longer triggers coaching (#529); `draft_writer_agent` TEEL/register dedup (English-only variant); `state_tracker` stage-6 `skill` key + `approval_gate: true`; duplicated Phase Boundary block in `citation_compliance_agent` deduped to the post-#294 canonical sentence (5a → Phase 5, `abstract_bilingual_agent` references removed).
- `bibliography_agent.md` F2 SHA pin recomputed for the merged content; CHANGELOG release headings normalized to `## [x.y.z] - YYYY-MM-DD — title` for the vendored gates.
- Sync completion (2026-07-21): `docs/SETUP.md` fork-adapted end-to-end (eight skills across all install methods, fork URLs, Method 5 requires v3.20.0+ of this fork); auto-merged Korean trigger keywords stripped from `academic-paper-reviewer` description and Korean fixture rows dropped from the routing-fixture README; Stage 6 wording English-only on the state-machine surface (boundary-lint pins + the five content locks updated in lockstep); `tools/validate_schemas.py` heading regex accepts the Schema 19/20 em-dash form (pre-existing FAIL → PASS); `shared/schema_migrations.md` compatibility-matrix row order + missing column fixed.

**Version bumps**: `academic-pipeline` 3.19.0 → 3.20.0 (suite-pinned); plugin + marketplace 3.19.0 → 3.20.0. Other skill versions unchanged.

## [3.19.0] - 2026-06-21 — Upstream sync: v3.10 → v3.13.0 (Windows portability + provider-agnostic verification + citation-existence gate + new modes)

Curated content sync of upstream Imbad0202 v3.10 → v3.13.0 (`8ca9d25` → `c22c17e`) onto the fork's v3.18.0 baseline. Bilingual additions (README.zh-CN.md #181, README.ja-JP.md #161/#162, zh-TW docs) are NOT merged — fork remains English-only. Substantive feature/fix tracks merged; pure-docs design specs and eval gold sets brought along for lint consistency. Fork priorities preserved throughout: English-only output, the experiment pipeline (Schemas 10–18), and existing Windows guards.

**Windows / portability:**
- Windows Python hook portability + graceful no-Python degradation (#454, #451). New `.gitattributes` pins `*.sh` to LF so Git Bash hooks (`hooks/run_guard.sh`, `scripts/announce-ars-loaded.sh`) survive `core.autocrlf=true` checkouts.
- `agents/` plugin-shipped agents materialized as **real byte-copies** of their `deep-research/agents/` sources (#413), replacing relative symlinks (which break on Windows). New `scripts/check_agents_mirror_sync.py` lint pins them as pure aliases.

**Verification + integrity:**
- Provider-agnostic cross-model verification — OpenAI-compatible endpoints (DeepSeek, MiMo, self-hosted) alongside Gemini (#455). New `scripts/cross_model_verification/` jq + normalizer helpers.
- `citation_existence` terminal policy + arXiv resolver + verification cache (#182): `scripts/arxiv_client.py`, `scripts/verification_cache.py`, `scripts/verification_gate/`, `citation_verification_summary` schema.
- Cross-index triangulation policy layer (#127) + review-material trust-boundary hardening + encoded Crossref locators (#310).
- Experiment Provenance Intake + claim→experiment alignment (#260); Figure/Table Fidelity Gate (#261); cross-paper contradiction inventory (#262).

**New modes + commands:**
- `three-way-scan` + `rebuttal-audit` reviewer/paper modes (#433) with `/ars-3w`, `/ars-rebuttal-audit`, `/ars-cache-invalidate` commands.
- `format_profile` schema + intake follow-up + formatter consumption (#439, declared-values-only, byte-equivalence Invariant 7).
- Diff/patch revision mode (#89): `scripts/ars_anchorize_draft.py`, `scripts/ars_apply_revision_patch.py`, block/patch contracts, deterministic apply.
- Submission-package verifier CLI (#394): `scripts/verify_submission_package.py` (reference integrity, venue profile, blind-review residue, repro-lock assessment).
- Socratic adjacent-framing probe — STORM-borrowed perspective expansion (#461).

**Auto-research + evals:**
- Kong auto-research track + partial-evidence decomposition (#376); domain evidence profiles (#259); idea-diversity + version-family reconciliation advisories (#270/#267); sub-claim inventory before consensus (#214); commitment ledger (#256-A1).
- Phase-1 eval harness — `scripts/run_evals.py`, citation-extraction + field-norm + rq-framing + surface-form gold sets, ranking-lift gate (#263/#184/#215/#216).
- Many new `scripts/check_*.py` lint gates + `scripts/test_*.py` companions; root `pyproject.toml` (`pythonpath="."`) replaces root `conftest.py`; shared test helpers moved to `tests/` package (#311).

**Docs / infra:**
- `CITATION.cff` (#440), Zenodo DOI (#434), GitHub `copilot-instructions.md`, `gitleaks`-over-history repository-hygiene workflow (#151), eval-harness workflow.

**Conflict-resolution notes:**
- All bilingual upstream additions dropped (zh-CN / ja-JP READMEs, zh-TW docs).
- 45 fork-diverged files 3-way-merged against the `8ca9d25` content base; 16 + 10 doc conflicts resolved as fork-wins on version strings / English-only / fork structure, union for additive feature blocks (reviewer read-only IRON RULE, ethics/citation IRON RULEs, Rebuttal-Audit + Patch-Protocol sections, #260 carry-forward rows, sub-claim columns).
- `check_spec_consistency.py` / `test_check_spec_consistency.py` / `spec-consistency.yml` kept at fork (English-only, `check_bilingual_purge`) + new `check_rebuttal_audit_guard` call grafted in.
- `bibliography_agent.md` F2 SHA pin in `check_v3_9_4_temporal_verification.py` recomputed for the merged content (no temporal/M6/M5 ownership logic leaked).
- `academic-paper/SKILL.md` Agent Team preserved at 11 agents (fork count); upstream 12-agent inventory dropped. `examples/` inventory corrected to actual 9 files (dropped deleted `chinese_paper_example`).

**Version bumps:** `academic-pipeline` 3.18.0 → 3.19.0 (suite-pinned); `.claude-plugin/marketplace.json` corrected 3.17.0 → 3.19.0 (pre-existing drift). Other skill versions unchanged — fork keeps its own per-skill versions; new content is plumbed via opt-in flags + new files.
...
## [3.17.0-upstream] - 2026-07-16 — Pipeline boundary semantics, canonical cross-model handoff envelope, executable panel checker
...
## [3.16.0-upstream] - 2026-07-12 — Model tiering, cross-model gate hardening, WP advisory sharpening
...
## [3.15.0-upstream] - 2026-07-04 — Release-gate hardening, prompt-debt retirement round 2, defrift locks
...
## [3.14.0-upstream] - 2026-07-02 — Claude Science importability, eval-comment rendering, prompt-debt retirement
