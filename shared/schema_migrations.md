...
| v3.17.0–v3.20.1 | v1.0 | v1.0 | v1.0 | v1.0 | v1.0 / v1.1 | No numbered-schema changes. Passport sub-schemas added: claim-audit set (v3.18.0), citation-verification / provenance / terminal-policies set (v3.19.0). v3.20.0 upstream sync (v3.13.0 → v3.17.0) adds the degradation-registry contract but no schema changes. |
| v3.21.0 | v1.0 | v1.0 | v1.0 | v1.0 | v1.0 / v1.1 | No numbered-schema changes. Upstream sync v3.17.0 → v3.19.0+ adds the re-review contract family (`shared/contracts/re_review/`, incl. `traceability.schema.json`), extends Schema 20.1 with `eligible_roles` / `owner_role` role scoping (#574, additive), and adds the `read_scope` sidecar on the human-read ledger (#513, optional). All backwards-compatible. |
