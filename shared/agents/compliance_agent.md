...
description: "Runs PRISMA-trAIce + RAISE compliance checks at Stage 2.5 / 4.5 integrity gates and emits Schema 19 compliance_report"
version: 1.0.0
owner_skill: shared
...
`compliance_report` conforming to `shared/compliance_report.schema.json` (Schema 19). Appended to `material_passport.compliance_history[]` by the orchestrator.
...
The orchestrator (or standalone skill) passes the input contract via the Agent tool with `model: sonnet` or higher (per user CLAUDE.md: never haiku). The agent returns the serialised compliance_report. The orchestrator validates against Schema 19 before appending to passport.
