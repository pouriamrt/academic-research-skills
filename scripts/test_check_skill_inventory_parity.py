...
def _write_manifests(
    ...
    *,
    ...
    plugin = {
        ...
        else f"pipeline: {len(names)} skills, 9 modes",
    }
...
def test_header_and_separator_rows_are_not_data_rows(tmp_path: Path, separator: str) -> None:
    ...
    md = tmp_path / ".claude" / "CLAUDE.md"
    md.write_text(
        md.read_text(encoding="utf-8").replace("|-------|---------|-----------|", separator),
        encoding="utf-8",
    )
    assert _violations(tmp_path) == []
...
def test_escaped_pipe_in_header_is_one_cell(tmp_path: Path) -> None:
    ...
    md.write_text(
        md.read_text(encoding="utf-8").replace(
            "| Skill | Purpose | Key Modes |", "| Skill | Purpose \\| Role | Key Modes |"
        ),
        encoding="utf-8",
    )
    assert _violations(tmp_path) == []
...
def test_demoted_heading_is_not_the_section(tmp_path: Path) -> None:
    ...
    md = tmp_path / ".claude" / "CLAUDE.md"
    md.write_text(
        md.read_text(encoding="utf-8").replace("## Skills Overview", "### Skills Overview"),
        encoding="utf-8",
    )
    v = _violations(tmp_path)
...
def test_fenced_heading_copy_does_not_bind(tmp_path: Path) -> None:
    ...
    md.write_text(
        md.read_text(encoding="utf-8").replace(
            "## Skills Overview", fenced + "## Skills Overview", 1
        ),
        encoding="utf-8",
    )
    assert _violations(tmp_path) == []
...
def test_description_without_count_makes_no_claim(tmp_path: Path) -> None:
    _build_root(tmp_path)
    _write_manifests(
        tmp_path, NAMES, market_desc="research skills for Claude Code", plugin_desc="40 skillsets"
    )
    assert _violations(tmp_path) == []
