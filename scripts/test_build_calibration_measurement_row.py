...
def call_row(label, start, attempt=1):
    return {
        "call": label,
        "attempt": attempt,
        "started_at": f"2026-09-06T12:{start:02d}:00.000000Z",
        "completed_at": f"2026-09-06T12:{start + 1:02d}:00.000000Z",
        "outcome": "completed",
...
def write_raw(raw_dir: Path, labels) -> None:
    ...
    for label in labels:
        (raw_dir / f"{label}.md").write_text(output_for(label), encoding="utf-8", newline="\n")
...
def make_work(tmp_path: Path, *, dirty=False, blocked=True) -> Path:
        ...
        json.dumps(
            {
                ...
                "paper_id": "p1",
                "status": "complete",
                **provenance(suite_commit_dirty=dirty),
                "calls": [call_row("field_analyst", 0)],
    ...
    (runs / "2026-09-06-p1-r1.json").write_text(
        json.dumps(
            {
                "suite": "reviewer_calibration",
                "stage": "panel",
    ...
    if blocked:
        (runs / "blocked-2026-09-06-p2-r1.json").write_text(
            json.dumps(
                {
                    "suite": "reviewer_calibration",
                    "stage": "panel",
                    ...
                    "replicate": 1,
                    "status": "aborted",
                    "abort_reason": "TransportFailure: x",
                    **provenance(suite_commit_dirty=dirty),
                    "calls": [],
                    "raw_bundle": "runs/2026-09-06-p2-r1/raw",
                }
            ),
            encoding="utf-8",
        )
        ...
        json.dumps(
            {
                ...
                "tier": "full",
                "n_papers": 1,
                "gold_composition": {"accept": 1, "reject": 0},
                "runs_per_paper": replicates,
                "confusion_matrix": {"tp": 1, "fn": 0, "fp": 0, "tn": 0},
                "metrics": {"balanced_accuracy": None, "FNR_over_harsh": 0.0, "FPR_lenient": None},
                "bootstrap_95ci": {},
                "exact_label_agreement": {"count": 1, "share": 1.0},
                "replicate_stability": {"side_agreement_share": 1.0, "exact_agreement_share": 1.0},
                "auc": "NOT REPORTED",
                "blocked_runs": ["blocked-2026-09-06-p2-r1.json"],
                "input_bindings": bindings,
                "per_panel": {
                    k: {kk: v for kk, v in row.items() if kk != "paper_id"}
        ...
        rows.append(
            {
                ...
                "model_family": family,
                "prompt_ref": "judge_template_v1",
                "evidence_provided": "seat weakness text only",
                "judging_budget": "1 call per item",
...
def argv(tmp_path: Path, work: Path, metrics: Path, judges: Path, extra=()):
    return [
        ...
        "--class-a-audit",
        str(tmp_path / "class-a-audit.json"),
        "--judge-template-version",
        "judge_template_v1",
        "--measurement-date",
        "2026-09-06",
        "--runs-ref",
        "evals/heldout/reviewer_calibration/runs/2026-09-06-attempt-1",
        "--verdict",
        "harness rehearsal",
        "--out",
...
def pinned(monkeypatch):
    ...
    monkeypatch.setattr(
        mod, "sha256_at_commit", lambda commit, rel: mod.sha256_file(mod.REPO / rel)
    )
...
def test_row_builds_validates_and_is_write_once(tmp_path, pinned):
    ...
    args = argv(
        tmp_path,
        work,
        make_metrics(tmp_path),
        make_judges(tmp_path),
        [
            "--rehearsal",
            "--claim",
            "ordering",
            "--overrides",
            str(make_overrides(tmp_path)),
        ],
    )
    ...
    assert (
        row["preregistration"]["frozen_commit"] == HEAD == row["subject"]["config"]["suite_commit"]
    )
...
def test_stale_manifest_refuses(tmp_path, pinned):
    work = make_work(tmp_path)
    make_manifest(work)
    manifest = json.loads((work / "execution-manifest.json").read_text(encoding="utf-8"))
    manifest["calls"] = manifest["calls"][:-1]
    (work / "execution-manifest.json").write_text(json.dumps(manifest), encoding="utf-8")
...
def test_single_replicate_needs_written_exception(tmp_path, pinned, capsys):
    ...
    ok = argv(
        ...
        [
            "--replicate-exception",
            "harness rehearsal: one replicate exercises the pipeline only",
        ],
    )
...
def test_foreign_metrics_are_refused(tmp_path, pinned):
    ...
    payload["per_panel"] = {
        "p1-r1": {"replicate": 1, "attempt_id": "attempt-9", "decision": "Accept"}
    }
    metrics.write_text(json.dumps(payload), encoding="utf-8")
...
def test_edited_raw_output_is_refused(tmp_path, pinned):
    ...
    synthesis.write_text(
        synthesis.read_text(encoding="utf-8").replace("Major Revision", "Accept"), encoding="utf-8"
    )
    ...
    with pytest.raises(mod.PreconditionFailure, match="concurrency"):
        mod.main(
            argv(
                ...
                work,
                make_metrics(tmp_path),
                make_judges(tmp_path, diverge=False),
                ["--claim", "concurrency"],
            )
        )
    assert not (tmp_path / "row.json").exists()
...
def test_non_finite_metrics_are_refused(tmp_path, pinned):
    ...
    metrics.write_text(
        metrics.read_text(encoding="utf-8").replace(
            '"FNR_over_harsh": 0.0', '"FNR_over_harsh": NaN'
        ),
        encoding="utf-8",
    )
...
def test_judge_failure_ledger_satisfies_partial_coverage(tmp_path, pinned, capsys):
    ...
    make_manifest(work)
    judges = make_judges(tmp_path, diverge=False)
    rows = json.loads(judges.read_text(encoding="utf-8"))
    rows[1]["per_item"] = rows[1]["per_item"][:1]  # judge-2 never returned w2
    judges.write_text(json.dumps(rows), encoding="utf-8")
    assert mod.main(argv(tmp_path, work, make_metrics(tmp_path), judges)) == 1
    ...
    ok = argv(
        ...
        [
            "--blocked-run",
            "judge-2 exhausted its retry on item w2 (transport failure)",
        ],
    )
    assert mod.main(ok) == 0
    row = json.loads((tmp_path / "row.json").read_text(encoding="utf-8"))
...
@pytest.mark.parametrize(
    "mutation,match",
    [
        ("coverage", "complete panel coverage"),
        ("hash", "hash mismatch"),
        ("decision", "decision differs"),
        ("excerpt", "verbatim raw"),
        ("blinding", "blinding"),
        ("criterion", "criterion"),
    ],
)
def test_class_a_audit_must_cover_and_match_every_synthesis(tmp_path, pinned, mutation, match):
    ...
    if mutation == "coverage":
        ...
        field, value = {
            "hash": ("synthesis_sha256", SHA),
            "decision": ("decision", "Reject"),
            "excerpt": ("raw", "absent"),
            "criterion": ("criterion_ref", "A2"),
        }[mutation]
        entry[field] = value
...
def test_same_panel_ids_cannot_hide_different_scoring_inputs(tmp_path, pinned, changed):
    ...
    if changed == "gold":
        (tmp_path / "gold.json").write_text(
            json.dumps({"labels": [{"paper_id": "p1", "label": "reject"}]}), encoding="utf-8"
        )
    elif changed in ("overrides", "severity"):
        path = tmp_path / "new-input.json"
        path.write_text("{}" if changed == "overrides" else "[]", encoding="utf-8")
        extra = [
            "--decision-overrides" if changed == "overrides" else "--severity-classifications",
            str(path),
        ]
    else:
        payload = json.loads(metrics.read_text(encoding="utf-8"))
...
def test_class_a_correction_builds_without_a_fictitious_severity_judge_item(tmp_path, pinned):
    ...
    override_path.write_text(
        json.dumps({"p1-r1": {"decision": "Accept", "raw": "Final decision: Accept."}}),
        encoding="utf-8",
    )
    assert (
        mod.scorer.main(
    ...
    audit_path = tmp_path / "class-a-audit.json"
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    audit["panels"]["p1-r1"].update(
        decision="Accept", raw="Final decision: Accept.", criterion_ref="A3"
    )
    audit_path.write_text(json.dumps(audit), encoding="utf-8")
    assert (
        mod.main(
            argv(
                ...
                [
                    ...
                    str(override_path),
                    "--replicate-exception",
                    "one synthetic rehearsal replicate",
                ],
            )
        )
        == 0
