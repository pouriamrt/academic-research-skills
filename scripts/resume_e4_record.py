...
def _result_from_state(state: dict, bundle_root: Path) -> tuple[harness.PanelResult, dict]:
    ...
    for event in result.retries:
        if event.stage == "synthesis":
            ...
            journal_prefix = (
                f"RETRY {event.role} phase1 diagnostic="
                if event.stage == "phase1"
                else f"RETRY {event.role} phase2 multi-dissent diagnostic="
            )
    ...
    for stage in completed:
        ...
        if stage == "synthesis":
            ...
            pass_marker = (
                "PHASE1-CONFORMANCE: PASS" if phase == "phase1" else "PHASE-CONFORMANCE: PASS"
            )
