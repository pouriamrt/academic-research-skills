...
# Academic Pipeline v3.20.0 — Full Academic Research Workflow Orchestrator (suite-version-pinned, auto-by-default)
...
- Simultaneously, `academic-paper/argument_builder_agent` can build CER chains
- `academic-paper/draft_writer_agent` waits for both to complete before beginning Phase 4
...
**Terminal semantics (#528)**: Stage 6 is non-mandatory — the user may decline it at the Stage 5 completion checkpoint (Stage 6 marked `skipped`; the pipeline still terminates `completed`). When it runs, after the process record is delivered the orchestrator prompts for a terminal acknowledgement (interactive mode; AUTO mode proceeds as if acknowledged) — `finish` / `end` / `done` / `confirm`, or an unambiguous natural-language equivalent that accepts the deliverables. On acknowledgement, Stage 6 is marked `completed` and the pipeline global state is set to `completed`; change requests (content corrections) keep Stage 6 `in_progress` and are not acknowledgements. See `references/pipeline_state_machine.md` § Stage 6 terminal semantics.
...
|---------|------|---------|
| 2.7 | 2026-03-28 | **Experiment Pipeline Integration + Style Profile**: Full Stage 1.5 wiring (experiment-designer → data-analyst/simulation-runner → lab-notebook); Schema 10/11/12/13 consumption in pipeline orchestrator; auto-detection of experiment requirements via Methodology Blueprint routing flags (`requires_experiment_design`, `requires_simulation`); lab-notebook continuous logging during experiment stages; Schema 11/12 integration into academic-paper Stage 2 (Results + Methods sections). Pipeline orchestrator now carries optional Style Profile (Schema 17) through all stages — produced by academic-paper intake Step 10 when user provides past writing samples, consumed by draft_writer (Stage 2) and report_compiler (Stage 1) as soft writing voice guide |
