...
def test_precedence_higher_severity_wins_regardless_of_order():
    ...
    assert (
        cps.resolve_decision(conds, {"F2", "F1"}) == "editorial_decision=reject_or_major_revision"
    )
    assert (
        cps.resolve_decision(conds, {"F3", "F2"}) == "editorial_decision=major_revision"
    )  # F2 sev 70 > F3 60
...
def make_report(role="eic", scores=None, fired=None, decision="editorial_decision=accept"):
    scores = scores or ALL_PASS
    fired = fired if fired is not None else {"F1": False, "F2": False, "F3": False, "F0": True}
    ...
    parts += ["## Review Body", "", "Fixture body.", "", "## Editorial Decision", "", decision, ""]
    return "\n".join(parts)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda t: t.replace(
                "editorial_decision=accept", "editorial_decision=accept\neditorial_decision=accept"
            ),
            "decision",
        ),
...
def test_prose_embedded_decision_not_matched():
    ...
    text = make_report().replace(
        "Fixture body.", "Fixture body mentioning editorial_decision=reject inline in prose."
    )
    r = cps.parse_report("r.md", text, FULL_CONTRACT)
    assert r.decision == "editorial_decision=accept"
...
def test_parse_synthesis_happy_and_empty_list():
    fired, dec = cps.parse_synthesis(
        "s.md", make_synthesis(["F2"], "editorial_decision=major_revision"), FULL_CONTRACT
    )
...
def test_load_contract_shipped_templates(tmp_path):
    for rel in (
        ...
        assert set(predicates) == {c["condition_id"] for c in contract["failure_conditions"]}
...
def test_layer1_consistent_report_no_diags():
    r = cps.parse_report(
        "r.md",
        make_report(scores=WARN2, fired=FIRED_F2, decision="editorial_decision=major_revision"),
        FULL_CONTRACT,
    )
...
def test_layer1_fired_flag_contradicts_scores():
    r = cps.parse_report(
        ...
        make_report(
            scores=WARN2,
            fired=FIRED_F0,  # claims F0 despite 2 warns
...
def test_layer1_decision_contradicts_declared_fired():
    r = cps.parse_report(
        "r.md",
        make_report(scores=WARN2, fired=FIRED_F2, decision="editorial_decision=accept"),
        FULL_CONTRACT,
    )
    diags = cps.layer1_check(r, FULL_CONTRACT, _predicates(), [])
...
def test_layer1_zero_fired_fallback_reviewer_level():
    ...
    r = cps.parse_report(
        "r.md",
        make_report(scores=one_warn, fired=none_fired, decision="editorial_decision=accept"),
        FULL_CONTRACT,
    )
    assert cps.layer1_check(r, FULL_CONTRACT, _predicates(), []) == []
    ...
    return [
        cps.parse_report(
            ...
            make_report(
                ...
                fired=fired_by_role[role],
                decision=decision_by_role[role],
            ),
            FULL_CONTRACT,
        )
...
def _consistent_panel_majority_f2():
    scores = {
        ...
        "methodology": WARN2,
        "domain": WARN2,
        "perspective": ALL_PASS,
        "da": ALL_PASS,
    }
    fired = {
        "eic": FIRED_F2,
        "methodology": FIRED_F2,
        "domain": FIRED_F2,
        "perspective": FIRED_F0,
        "da": FIRED_F0,
    }
    dec = {
        "eic": "editorial_decision=major_revision",
...
def test_layer2_consistent_panel():
    ...
    diags = cps.layer2_check(
        reports, FULL_CONTRACT, _predicates(), ["F2"], "editorial_decision=major_revision", []
    )
    assert diags == []
...
def test_layer2_flipped_decision_fails():
    reports = _consistent_panel_majority_f2()
    diags = cps.layer2_check(
        reports, FULL_CONTRACT, _predicates(), ["F2"], "editorial_decision=minor_revision", []
    )
...
def test_layer2_fabricated_fired_list_fails_despite_right_decision():
    reports = _consistent_panel_majority_f2()
    diags = cps.layer2_check(
        reports, FULL_CONTRACT, _predicates(), ["F3"], "editorial_decision=major_revision", []
    )
    assert any("PANEL-SYNTHESIS-MISMATCH" in d for d in diags)
...
def test_layer2_panel_zero_fired_accepts():
    scores = {r: ALL_PASS for r in ("eic", "methodology", "domain", "perspective", "da")}
    ...
    diags = cps.layer2_check(
        reports, FULL_CONTRACT, _predicates(), [], "editorial_decision=accept", []
    )
    assert diags == []
...
def test_duplicate_path_emits_single_cardinality_diag(tmp_path, capsys):
    ...
    synth.write_text(make_synthesis([], "editorial_decision=accept"), encoding="utf-8")
    contract = str(REPO / "shared/contracts/reviewer/full.json")
    rc = cps.main(["--contract", contract] + ["--report", str(r)] * 5 + ["--synthesis", str(synth)])
    assert rc == 2
...
def cli(*extra, reports=None):
    ...
    for p in reports if reports is not None else [FIX / f"r_{r}.md" for r in ROLES]:
        argv += ["--report", str(p)]
    argv += list(extra)
...
def test_cli_flipped_synthesis_decision_exit1(tmp_path, capsys):
    ...
    bad.write_text(make_synthesis(["F2"], "editorial_decision=minor_revision"), encoding="utf-8")
...
def test_cli_inconsistent_reviewer_exit3(tmp_path):
    ...
    bad.write_text(
        make_report(role="eic", scores=WARN2, fired=FIRED_F0, decision="editorial_decision=accept"),
        encoding="utf-8",
    )
...
def test_cli_byte_identical_contents_exit2(tmp_path):
    ...
    clone.write_text((FIX / "r_eic.md").read_text(encoding="utf-8"), encoding="utf-8")
...
def test_cli_role_set_mismatch_exit2(tmp_path):
    ...
    dup_role.write_text(
        make_report(
            role="eic",
            scores={**ALL_PASS, "D3": "warn"},
            fired={"F1": False, "F2": False, "F3": False, "F0": False},
            decision="editorial_decision=accept",
        ),
        encoding="utf-8",
    )
...
def test_cli_exit_precedence_2_beats_3_and_1(tmp_path):
    ...
    synth = tmp_path / "synth.md"
    synth.write_text(make_synthesis([], "editorial_decision=reject"), encoding="utf-8")
...
def test_cli_methodology_focus_round(tmp_path, capsys):
    ...
    def mreport(role, d1, fired, decision):
        return "\n".join(
            [
                ...
                "",
                "### F1",
                f"fired: {str(fired['F1']).lower()}",
                "",
                "### F2",
                f"fired: {str(fired['F2']).lower()}",
                "",
                "### F0",
                f"fired: {str(fired['F0']).lower()}",
                "",
                "## Review Body",
                "",
                "Fixture body.",
    ...
    r1.write_text(
        mreport("eic", "pass", {"F1": False, "F2": False, "F0": True}, "editorial_decision=accept"),
        encoding="utf-8",
    )
    ...
    synth = tmp_path / "synth.md"
    synth.write_text(make_synthesis(["F2"], "editorial_decision=major_revision"), encoding="utf-8")
    assert (
        cps.main(
            [
                "--contract",
...
def test_unicode_line_separators_do_not_create_anchored_lines():
    ...
    mangled = base.replace(real, "prose\x85editorial_decision=accept\x85tail")
