...
_ATOM_RES = (
    ...
    (
        ...
        re.compile(
            ...
            r"|dimensions with priority=(?P<p2>[a-z]+)) score " + _SCORE_PART + r" or worse$"
        ),
    ),
    ("every_priority", re.compile(r"^every (?P<p1>[a-z]+) dimension scores " + _SCORE_PART + r"$")),
...
def parse_expression(expression, dims_by_priority, dim_ids, condition_id):
    ...
    for part in expression.split(" AND "):
        for kind, rx in _ATOM_RES:
            ...
            raise ContractError(
                f"[EXPRESSION-UNRECOGNISED: condition_id={condition_id}, expression={expression}]"
            )
        score = m.group("score")
        if kind == "dim_literal":
            dim = m.group("dim")
            if dim not in dim_ids:
                raise ContractError(
                    f"[EXPRESSION-SEMANTIC: condition_id={condition_id}: unknown dimension {dim}]"
                )
        ...
        if kind == "any_priority":
            atoms.append(lambda scores, ds=scoped, s=score: any(scores[d] == s for d in ds))
        elif kind == "count_priority":
            floor = SCORE_ORDER[score]
            atoms.append(
                lambda scores, ds=scoped, f=floor: (
                    sum(1 for d in ds if SCORE_ORDER[scores[d]] >= f) >= 2
                )
            )
        else:  # every_priority
            atoms.append(lambda scores, ds=scoped, s=score: all(scores[d] == s for d in ds))
...
def quantifier_fires(quant, per_reviewer, warnings):
    ...
    if quant == "majority":
        if n == 1:
            warnings.append(
                "WARNING: majority quantifier with panel_size=1 never fires (protocol §8)"
            )
            return False
...
def resolve_decision(conditions, fired_ids):
    fired = [(i, c) for i, c in enumerate(conditions) if c["condition_id"] in fired_ids]
    if not fired:
...
REQUIRED_REPORT_SECTIONS = ("Dimension Scores", "Failure Condition Checks", "Editorial Decision")
...
def _exactly_one(lines, rx, what, path, group):
    ...
    if len(hits) != 1:
        raise ReportError(
            f"[REPORT-PARSE: {path}: expected exactly one {what} line, found {len(hits)}]"
        )
    return hits[0]
...
def parse_report(path, text, contract):
    ...
    for req in REQUIRED_REPORT_SECTIONS:
        if req in dupes:
            raise ReportError(f"[REPORT-PARSE: {path}: duplicated required section '## {req}']")
        if req not in sections:
            raise ReportError(f"[REPORT-PARSE: {path}: missing required section '## {req}']")
    ...
    if sub_dupes:
        raise ReportError(
            f"[REPORT-PARSE: {path}: duplicated Dimension Scores subsection(s) {sorted(sub_dupes)}]"
        )
    scores = {}
    for title, sublines in subs.items():
        ...
        if not m or m.group("dim") not in dim_ids:
            raise ReportError(
                f"[REPORT-PARSE: {path}: unknown Dimension Scores subsection '### {title}']"
            )
    ...
    if missing:
        raise ReportError(f"[REPORT-PARSE: {path}: missing Dimension Scores for {missing}]")
    ...
    for title, sublines in fsubs.items():
        if title not in cond_ids:
            raise ReportError(
                f"[REPORT-PARSE: {path}: unknown Failure Condition Checks subsection '### {title}']"
            )
        fired[title] = (
            _exactly_one(sublines, _FIRED_LINE_RE, f"fired ({title})", path, "fired") == "true"
        )
    missing = [c for c in cond_ids if c not in fired]
    if missing:
        raise ReportError(f"[REPORT-PARSE: {path}: missing Failure Condition Checks for {missing}]")

    decision = _exactly_one(lines, _DECISION_LINE_RE, "decision", path, "action")
    if decision not in ACTION_ENUM:
        raise ReportError(f"[REPORT-PARSE: {path}: unknown decision action token '{decision}']")
    in_section = [
        m.group("action")
        for line in sections["Editorial Decision"]
        if (m := _DECISION_LINE_RE.match(line))
    ]
    if len(in_section) != 1:
        raise ReportError(
            f"[REPORT-PARSE: {path}: decision line must sit inside '## Editorial Decision']"
        )
    return ReviewerReport(path=path, role=role, scores=scores, fired=fired, decision=decision)
...
def parse_synthesis(path, text, contract):
    lines = strip_fences(text)
    bodies = [m.group("body") for line in lines if (m := _FIREDLIST_RE.match(line))]
    if len(bodies) != 1:
    ...
    if unknown:
        raise SynthesisError(f"[SYNTHESIS-PARSE: {path}: unknown condition id(s) {unknown}]")
    if len(fired) != len(set(fired)):
        raise SynthesisError(f"[SYNTHESIS-PARSE: {path}: duplicate condition id in fired list]")
    decisions = [m.group("action") for line in lines if (m := _DECISION_LINE_RE.match(line))]
    if len(decisions) != 1:
        raise SynthesisError(
            f"[SYNTHESIS-PARSE: {path}: expected exactly one decision line, found {len(decisions)}]"
        )
    if decisions[0] not in ACTION_ENUM:
        raise SynthesisError(
            f"[SYNTHESIS-PARSE: {path}: unknown decision action token '{decisions[0]}']"
        )
...
def load_contract(path):
    ...
    if problems:
        raise ContractError(f"[CONTRACT-INVALID: {path}: " + "; ".join(problems) + "]")
...
def layer2_check(reports, contract, predicates, declared_fired, declared_decision, warnings):
    recomputed_fired = []
    for cond in contract["failure_conditions"]:
        cid = cond["condition_id"]
        per_reviewer = [predicates[cid](r.scores) for r in reports]
        if quantifier_fires(cond["cross_reviewer_quantifier"], per_reviewer, warnings):
            recomputed_fired.append(cid)
    recomputed_decision = resolve_decision(contract["failure_conditions"], set(recomputed_fired))
    if (
        sorted(recomputed_fired) != sorted(declared_fired)
...
def _parse_args(argv):
    ...
    parser.add_argument("--report", required=True, action="append", type=Path, dest="reports")
...
def main(argv=None):
    ...
    if not args.layer1_only and len(args.reports) != panel_size:
        infra.append(f"[PANEL-CARDINALITY: got={len(args.reports)}, panel_size={panel_size}]")
    ...
    for p in args.reports:
        ...
        if digest in hashes:
            infra.append(f"[PANEL-CARDINALITY: byte-identical report contents ({p})]")
        hashes.add(digest)
    ...
    if not parse_failed:
        if args.layer1_only:
            ...
            if bad or len(set(roles)) != len(roles):
                infra.append(
                    f"[PANEL-CARDINALITY: roles {roles} invalid for mode {contract['mode']}]"
                )
        elif set(roles) != role_set or len(set(roles)) != len(roles):
            infra.append(
                f"[PANEL-CARDINALITY: roles {sorted(roles)} != required {sorted(role_set)}]"
            )
    ...
    if not args.layer1_only:
        if parse_failed or infra:
            synthesis_diags.append(
                "[SUPPRESSED: panel recomputation skipped — upstream reviewer/cardinality failure]"
            )
        else:
            try:
                ...
                synthesis_diags.extend(
                    layer2_check(
                        reports, contract, predicates, declared_fired, declared_decision, warnings
                    )
                )
    ...
    print("PANEL-SYNTHESIS: PASS" if not args.layer1_only else "LAYER1-ONLY: PASS")
    return EXIT_PASS
